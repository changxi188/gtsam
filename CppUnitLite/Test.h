...
protected:
    ...
    bool check(const std::string& expected, const std::string& actual, TestResult& result, const std::string& fileName,
               long lineNumber);
...
#define TEST(testGroup, testName)                                                                                      \
    ...
    {                                                                                                                  \
    public:                                                                                                            \
        testGroup##testName##Test() : Test(#testName "Test", __FILE__, __LINE__, true)                                 \
        {                                                                                                              \
        }                                                                                                              \
...
#define TEST_UNSAFE(testGroup, testName)                                                                               \
    ...
    {                                                                                                                  \
    public:                                                                                                            \
        testGroup##testName##Test() : Test(#testName "Test", __FILE__, __LINE__, false)                                \
        {                                                                                                              \
        }                                                                                                              \
        virtual ~testGroup##testName##Test(){};                                                                        \
        void run(TestResult& result_) override;                                                                        \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            result_.addFailure(Failure(name_, __FILE__, __LINE__,                                                      \
                                       std::string("Didn't throw: ") + boost::lexical_cast<std::string>(#condition))); \
            return;                                                                                                    \
        }                                                                                                              \
        catch (...)                                                                                                    \
        {                                                                                                              \
...
#define CHECK_EXCEPTION(condition, exception_name)                                                                     \
    {                                                                                                                  \
        ...
        {                                                                                                              \
            result_.addFailure(Failure(name_, __FILE__, __LINE__,                                                      \
                                       std::string("Wrong exception: ") +                                              \
                                           boost::lexical_cast<std::string>(#condition) +                              \
                                           boost::lexical_cast<std::string>(", expected: ") +                          \
                                           boost::lexical_cast<std::string>(#exception_name)));                        \
            return;                                                                                                    \
        }                                                                                                              \
    }
...
#define CHECK_EQUAL(expected, actual)                                                                                  \
    {                                                                                                                  \
        if ((expected) == (actual))                                                                                    \
            return;                                                                                                    \
        result_.addFailure(Failure(name_, __FILE__, __LINE__, boost::lexical_cast<std::string>(expected),              \
...
#define DOUBLES_EQUAL(expected, actual, threshold)                                                                     \
    {                                                                                                                  \
        ...
        if (!std::isfinite(actualTemp) || !std::isfinite(expectedTemp) ||                                              \
            fabs((expectedTemp) - (actualTemp)) > threshold)                                                           \
        {                                                                                                              \
            result_.addFailure(Failure(name_, __FILE__, __LINE__,                                                      \
                                       boost::lexical_cast<std::string>((double)expectedTemp),                         \
                                       boost::lexical_cast<std::string>((double)actualTemp)));                         \
            return;                                                                                                    \
        }                                                                                                              \
    }

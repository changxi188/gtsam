...
public:
    ...
    SymmetricBlockMatrix(const CONTAINER& dimensions, bool appendOneDimension = false) : blockStart_(0)
    {
      ...
      : blockStart_(0)
    {
        ...
        if (variableColOffsets_.back() != matrix_.cols())
            throw std::invalid_argument("Requested to create a SymmetricBlockMatrix with dimensions that do not sum to "
                                        "the total size of the provided matrix.");
        assertInvariants();
    }
    ...
    constBlock aboveDiagonalRange(DenseIndex i_startBlock, DenseIndex i_endBlock, DenseIndex j_startBlock,
                                  DenseIndex j_endBlock) const
    {
        assert(i_startBlock < j_startBlock);
        assert(i_endBlock <= j_startBlock);
        return block_(i_startBlock, j_startBlock, i_endBlock - i_startBlock, j_endBlock - j_startBlock);
    }

    /// Get a range [i,j) from the matrix. Indices are in block units.
    Block aboveDiagonalRange(DenseIndex i_startBlock, DenseIndex i_endBlock, DenseIndex j_startBlock,
...
protected:
    ...
    {
        const std::array<DenseIndex, 4> indices = calcIndices(iBlock, jBlock, blockRows, blockCols);
    ...
    std::array<DenseIndex, 4> calcIndices(DenseIndex iBlock, DenseIndex jBlock, DenseIndex blockRows,

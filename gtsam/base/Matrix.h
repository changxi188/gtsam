...
void insertSub(Eigen::MatrixBase<Derived1>& fullMatrix, const Eigen::MatrixBase<Derived2>& subMatrix, size_t i,
               size_t j)
{
...
struct Reshape
{
    // TODO replace this with Eigen's reshape function as soon as available. (There is a PR already pending :
...
/// Reshape specialization that does nothing as shape stays the same (needed to not be ambiguous for square input equals
/// square output)
...
template <int OutM, int OutN, int OutOptions, int InM, int InN, int InOptions>
inline typename Reshape<OutM, OutN, OutOptions, InM, InN, InOptions>::ReshapedType
 ...
 */
GTSAM_EXPORT std::list<boost::tuple<Vector, double, double> > weighted_eliminate(Matrix& A, Vector& b,
                                                                                 const Vector& sigmas);
 ...
 */
GTSAM_EXPORT boost::tuple<int, double, Vector> DLT(const Matrix& A, double rank_tol = 1e-9);
...
struct MultiplyWithInverse
{
    ...
    VectorN operator()(const MatrixN& A, const VectorN& b, OptionalJacobian<N, N* N> H1 = boost::none,
...
struct MultiplyWithInverseFunction
{
    ...
    VectorN operator()(const T& a, const VectorN& b, OptionalJacobian<N, M> H1 = boost::none,
                       OptionalJacobian<N, N> H2 = boost::none) const
    {
        MatrixN A;

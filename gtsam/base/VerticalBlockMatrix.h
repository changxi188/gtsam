...
public:
    ...
    VerticalBlockMatrix(const CONTAINER& dimensions, const Eigen::MatrixBase<DERIVED>& matrix,
      ...
      , blockStart_(0)
    {
        ...
        if (variableColOffsets_.back() != matrix_.cols())
            throw std::invalid_argument("Requested to create a VerticalBlockMatrix with dimensions that do not sum to "
                                        "the total columns of the provided matrix.");
        assertInvariants();
    }
    ...
    /** Get the apparent last row (exclusive, i.e. rows() == rowEnd() - rowStart()) of the underlying matrix for all
     * operations */
    ...
    /** Get or set the apparent last row (exclusive, i.e. rows() == rowEnd() - rowStart()) of the underlying matrix for
     * all operations */
    DenseIndex& rowEnd()
    {
        return rowEnd_;

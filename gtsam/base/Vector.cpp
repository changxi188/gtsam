...
bool fpEqual(double a, double b, double tol, bool check_relative_also)
{
    ...
    else if (abs(a - b) <= tol * min(larger, std::numeric_limits<double>::max()) && check_relative_also)
    {
        return true;
    }
...
{
    if (equal_with_abs_tol(expected, actual, tol))
        return true;
    cout << "not equal:" << endl;
...
{
    if (equal_with_abs_tol(Vector(expected), Vector(actual), tol))
        return true;
    cout << "not equal:" << endl;
...
double weightedPseudoinverse(const Vector& a, const Vector& weights, Vector& pseudo)
{
    ...
    vector<bool> isZero;
    for (size_t i = 0; i < m; ++i)
        isZero.push_back(std::abs(a[i]) < 1e-9);

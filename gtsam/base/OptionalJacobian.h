...
public:
    ...
    OptionalJacobian(Jacobian& fixed) : map_(nullptr)
    {
        usurp(fixed.data());
    }
    ...
    OptionalJacobian(Eigen::Ref<MATRIX> dynamic_ref) : map_(nullptr)
    {
        ...
        {
            throw std::invalid_argument(std::string("OptionalJacobian called with wrong dimensions or "
                                                    "storage order.\n"
                                                    "Expected: ") +
    ...
    OptionalJacobian(const boost::optional<Eigen::MatrixXd&> optional) : map_(nullptr)
    {
        if (optional)
        {
...
public:
    ...
    /// Constructor compatible with old-style derivatives
    OptionalJacobian(const boost::optional<Eigen::MatrixXd&> optional) : pointer_(nullptr)
    {
        if (optional)
            pointer_ = &(*optional);
    }

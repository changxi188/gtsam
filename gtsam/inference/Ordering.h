...
public:
    ...
    template <class FACTOR_GRAPH>
    static Ordering ColamdConstrainedLast(const FACTOR_GRAPH& graph, const KeyVector& constrainLast,
    ...
    template <class FACTOR_GRAPH>
    static Ordering ColamdConstrainedFirst(const FACTOR_GRAPH& graph, const KeyVector& constrainFirst,
                                           bool forceOrder = false)
    {
        if (graph.empty())
            return Ordering();
    ...
    static Ordering Create(OrderingType orderingType, const FACTOR_GRAPH& graph)
    {
        ...
        switch (orderingType)
        {
            ...
            case CUSTOM:
                throw std::runtime_error("Ordering::Create error: called with CUSTOM ordering type.");
            default:
                throw std::runtime_error("Ordering::Create error: called with unknown ordering type.");
        }
    }

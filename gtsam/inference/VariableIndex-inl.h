...
/* ************************************************************************* */
template <class FG>
void VariableIndex::augment(const FG& factors, boost::optional<const FactorIndices&> newFactorIndices)
{
    ...
    for (size_t i = 0; i < factors.size(); ++i)
    {
        if (factors[i])
        {
            const size_t globalI = newFactorIndices ? (*newFactorIndices)[i] : nFactors_;
    ...
    for (; factorIndex != lastFactor; ++factorIndex, ++i)
    {
        ...
        if (factors[i])
        {
            for (Key j : *factors[i])
            {
                ...
                auto           entry         = std::find(factorEntries.begin(), factorEntries.end(), *factorIndex);
                if (entry == factorEntries.end())
                    throw std::invalid_argument("Internal error, indices and factors passed into VariableIndex::remove "
                                                "are not consistent with the existing variable index");

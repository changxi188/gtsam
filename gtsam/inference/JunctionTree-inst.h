...
struct ConstructorTraversalData
{
    ...
    static ConstructorTraversalData ConstructorTraversalVisitorPre(const boost::shared_ptr<ETREE_NODE>& node,
                                                                   ConstructorTraversalData&            parentData)
    {
        ...
        myData.junctionTreeNode         = boost::make_shared<Node>(node->key, node->factors);
        parentData.junctionTreeNode->addChild(myData.junctionTreeNode);
        return myData;
    }

    // Post-order visitor function
    static void ConstructorTraversalVisitorPostAlg2(const boost::shared_ptr<ETREE_NODE>& ETreeNode,
                                                    const ConstructorTraversalData&      myData)
    {
        ...
        boost::tie(myConditional, mySeparatorFactor) = internal::EliminateSymbolic(symbolicFactors, keyAsOrdering);
        ...
        const FastVector<SymbolicConditional::shared_ptr>& childConditionals = myData.childSymbolicConditionals;
...
JunctionTree<BAYESTREE, GRAPH>::JunctionTree(const EliminationTree<ETREE_BAYESNET, ETREE_GRAPH>& eliminationTree)
{
    ...
    treeTraversal::DepthFirstForest(eliminationTree, rootData, Data::ConstructorTraversalVisitorPre,

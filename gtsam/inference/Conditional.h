...
public:
    ...
    {
        return asFactor().size() - nrFrontals_;
    }
    ...
    typename FACTOR::const_iterator beginFrontals() const
    {
    ...
    typename FACTOR::const_iterator beginParents() const
    {
        return endFrontals();
    }

    /** Iterator pointing past the last parent key. */
    typename FACTOR::const_iterator endParents() const
    {
        return asFactor().end();
    }
    ...
    typename FACTOR::iterator beginFrontals()
    {
        return asFactor().begin();
    }
    ...
    typename FACTOR::iterator endParents()
    {
        return asFactor().end();
    }
...
private:
    ...
    FACTOR& asFactor()
    {
        return static_cast<FACTOR&>(static_cast<DERIVEDCONDITIONAL&>(*this));
    }

    // Cast to derived type (const) (casts down to derived conditional type, then up to factor type)
    const FACTOR& asFactor() const
    {
        return static_cast<const FACTOR&>(static_cast<const DERIVEDCONDITIONAL&>(*this));
    }

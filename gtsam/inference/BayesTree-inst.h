...
BayesTreeCliqueData BayesTree<CLIQUE>::getCliqueData() const
{
    ...
    for (const sharedClique& root : roots_)
        getCliqueData(root, &stats);
    return stats;
}

/* ************************************************************************* */
template <class CLIQUE>
void BayesTree<CLIQUE>::getCliqueData(sharedClique clique, BayesTreeCliqueData* stats) const
{
...
/* ************************************************************************* */
template <class CLIQUE>
void BayesTree<CLIQUE>::dot(std::ostream& os, const KeyFormatter& keyFormatter) const
{
    if (roots_.empty())
        throw std::invalid_argument("the root of Bayes tree has not been initialized!");
    os << "digraph G{\n";
    for (const sharedClique& root : roots_)
        dot(os, root, keyFormatter);
    os << "}";
    std::flush(os);
}
...
/* ************************************************************************* */
template <class CLIQUE>
void BayesTree<CLIQUE>::saveGraph(const std::string& filename, const KeyFormatter& keyFormatter) const
{
...
/* ************************************************************************* */
template <class CLIQUE>
void BayesTree<CLIQUE>::dot(std::ostream& s, sharedClique clique, const KeyFormatter& keyFormatter, int parentnum) const
{
    static int        num   = 0;
    bool              first = true;
...
boost::shared_ptr<NODE> BayesTreeCloneForestVisitorPre(const boost::shared_ptr<NODE>& node,
...
template <class CLIQUE>
bool check_sharedCliques(const std::pair<Key, typename BayesTree<CLIQUE>::sharedClique>& v1,
...
/* ************************************************************************* */
template <class CLIQUE>
typename BayesTree<CLIQUE>::sharedConditional BayesTree<CLIQUE>::marginalFactor(Key j, const Eliminate& function) const
{
    ...
    BayesNetType marginalBN = *cliqueMarginal.marginalMultifrontalBayesNet(Ordering{j}, function);
...
/* ************************************************************************* */
template <class CLIQUE>
typename BayesTree<CLIQUE>::sharedFactorGraph BayesTree<CLIQUE>::joint(Key j1, Key j2, const Eliminate& function) const
{
...
/* ************************************************************************* */
template <class CLIQUE>
typename BayesTree<CLIQUE>::sharedBayesNet BayesTree<CLIQUE>::jointBayesNet(Key j1, Key j2,
...
/* ************************************************************************* */
template <class CLIQUE>
typename BayesTree<CLIQUE>::Cliques BayesTree<CLIQUE>::removeSubtree(const sharedClique& subtree)
{
    // Result clique list
    Cliques cliques;
    ...
    if (!subtree->isRoot())
        subtree->parent()->children.erase(
            std::find(subtree->parent()->children.begin(), subtree->parent()->children.end(), subtree));
    else

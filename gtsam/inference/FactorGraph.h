...
private:
    ...
    template <typename T>
    using HasDerivedValueType =
        typename std::enable_if<std::is_base_of<FactorType, typename T::value_type>::value>::type;

    /// Check if T has a pointer type derived from FactorType.
    template <typename T>
    using HasDerivedElementType =
        typename std::enable_if<std::is_base_of<FactorType, typename T::value_type::element_type>::value>::type;
...
public:
    ...
    FactorGraph(std::initializer_list<boost::shared_ptr<DERIVEDFACTOR>> sharedFactors) : factors_(sharedFactors)
    {
    }
    ...
    {
        factors_.push_back(boost::allocate_shared<DERIVEDFACTOR>(Eigen::aligned_allocator<DERIVEDFACTOR>(),
...
public:
    ...
    inline bool exists(size_t idx) const
    {

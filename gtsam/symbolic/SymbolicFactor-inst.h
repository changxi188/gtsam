...
EliminateSymbolic(const FactorGraph<FACTOR>& factors, const Ordering& keys)
{
    ...
    std::set_difference(allKeys.begin(), allKeys.end(), frontals.begin(), frontals.end(),
                        orderedKeys.begin() + nFrontals);

    // Return resulting conditional and factor
    return std::make_pair(SymbolicConditional::FromKeysShared(orderedKeys, nFrontals),

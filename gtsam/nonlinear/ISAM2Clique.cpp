...
bool ISAM2Clique::equals(const This& other, double tol) const
{
    return Base::equals(other) &&
           ((!cachedFactor_ && !other.cachedFactor_) ||
            (cachedFactor_ && other.cachedFactor_ && cachedFactor_->equals(*other.cachedFactor_, tol)));
}
...
/* ************************************************************************* */
bool ISAM2Clique::valuesChanged(const KeySet& replaced, const Vector& originalValues, const VectorValues& delta,
                                double threshold) const
{
    auto frontals = conditional_->frontals();
    if (replaced.exists(frontals.front()))
        return true;
...
void ISAM2Clique::optimizeWildfire(const KeySet& replaced, double threshold, KeySet* changed, VectorValues* delta,
                                   size_t* count) const
{
...
size_t optimizeWildfire(const ISAM2Clique::shared_ptr& root, double threshold, const KeySet& keys, VectorValues* delta)
{
    ...
    if (root)
        root->optimizeWildfire(keys, threshold, &changed, delta, &count);
    return count;
}

/* ************************************************************************* */
bool ISAM2Clique::optimizeWildfireNode(const KeySet& replaced, double threshold, KeySet* changed, VectorValues* delta,
                                       size_t* count) const
{
    // TODO(gareth): This code shares a lot of logic w/ linearAlgorithms-inst,
    // potentially refactor
...
size_t optimizeWildfireNonRecursive(const ISAM2Clique::shared_ptr& root, double threshold, const KeySet& keys,
                                    VectorValues* delta)
{
    KeySet changed;
    size_t count = 0;
...
void ISAM2Clique::findAll(const KeySet& markedMask, KeySet* keys) const
{
    ...
    if (found)
    {
        ...
        if (debug)
            print("Key(s) marked in clique ");
        if (debug)
            cout << "so marking key " << conditional_->front() << endl;
    }
    for (const auto& child : children)
    {

...
struct ValuesCastHelper
{
    static CastedKeyValuePairType cast(KeyValuePairType key_value)
    {
        ...
        return CastedKeyValuePairType(
            key_value.key,
            const_cast<GenericValue<ValueType>&>(static_cast<const GenericValue<ValueType>&>(key_value.value)).value());
    }
};
...
/* ************************************************************************* */
template <class ValueType>
std::map<Key, ValueType> Values::extract(const std::function<bool(Key)>& filterFcn) const
{
    ...
    for (const auto& key_value : values_)
    {
        ...
        if (filterFcn(key_value.first))
        {
            ...
            if (auto t = dynamic_cast<const GenericValue<ValueType>*>(key_value.second))
...
public:
    ...
    typedef boost::transform_iterator<
        ...
        boost::filter_iterator<std::function<bool(const Values::ConstKeyValuePair&)>, Values::iterator>>
        iterator;
    ...
    typedef boost::transform_iterator<
        ...
        boost::filter_iterator<std::function<bool(const Values::ConstKeyValuePair&)>, Values::const_iterator>>
...
private:
    Filtered(const std::function<bool(const Values::ConstKeyValuePair&)>& filter, Values& values)
      : begin_(boost::make_transform_iterator(boost::make_filter_iterator(filter, values.begin(), values.end()),
                                              &ValuesCastHelper<ValueType, KeyValuePair, Values::KeyValuePair>::cast))
      , end_(boost::make_transform_iterator(boost::make_filter_iterator(filter, values.end(), values.end()),
                                            &ValuesCastHelper<ValueType, KeyValuePair, Values::KeyValuePair>::cast))
      , constBegin_(boost::make_transform_iterator(
            boost::make_filter_iterator(filter, values._begin(), values._end()),
            &ValuesCastHelper<ValueType, ConstKeyValuePair, Values::ConstKeyValuePair>::cast))
      , constEnd_(boost::make_transform_iterator(
            boost::make_filter_iterator(filter, values._end(), values._end()),
            &ValuesCastHelper<ValueType, ConstKeyValuePair, Values::ConstKeyValuePair>::cast))
    {
    }
...
public:
    ...
    ConstFiltered(const Filtered<ValueType>& rhs) : begin_(rhs.beginConst()), end_(rhs.endConst())
    {
    }
...
private:
    ...
    const_iterator end_;
    ConstFiltered(const std::function<bool(const Values::ConstKeyValuePair&)>& filter, const Values& values)
    {
...
Values::Filtered<Value> inline Values::filter(const std::function<bool(Key)>& filterFcn)
{
    return filter<Value>(filterFcn);
}

template <class ValueType>
Values::Filtered<ValueType> Values::filter(const std::function<bool(Key)>& filterFcn)
{
    return Filtered<ValueType>(std::bind(&filterHelper<ValueType>, filterFcn, std::placeholders::_1), *this);
}

Values::ConstFiltered<Value> inline Values::filter(const std::function<bool(Key)>& filterFcn) const
{
    return filter<Value>(filterFcn);
}

template <class ValueType>
Values::ConstFiltered<ValueType> Values::filter(const std::function<bool(Key)>& filterFcn) const
{
    return ConstFiltered<ValueType>(std::bind(&filterHelper<ValueType>, filterFcn, std::placeholders::_1), *this);
}
#endif

/* ************************************************************************* */
template <>
inline bool Values::filterHelper<Value>(const std::function<bool(Key)> filter, const ConstKeyValuePair& key_value)
{
...
struct handle<Eigen::Matrix<double, M, N>>
{
    ...
    {
        return handle_matrix<Eigen::Matrix<double, M, N>, (M == Eigen::Dynamic || N == Eigen::Dynamic)>()(j, pointer);
    }
};

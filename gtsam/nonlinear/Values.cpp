...
bool Values::equals(const Values& other, double tol) const
{
    if (this->size() != other.size())
        return false;
    for (auto it1 = values_.begin(), it2 = other.values_.begin(); it1 != values_.end(); ++it1, ++it2)
    {
        const Value* value1 = it1->second;
        const Value* value2 = it2->second;
        if (typeid(*value1) != typeid(*value2) || it1->first != it2->first || !value1->equals_(*value2, tol))
        {
            return false;
        }
    }
...
#else
    for (key_delta = delta.begin(); key_value != values_.end(); ++key_value, ++key_delta)
    {
...
VectorValues Values::localCoordinates(const Values& cp) const
{
    ...
    VectorValues result;
    for (auto it1 = values_.begin(), it2 = cp.values_.begin(); it1 != values_.end(); ++it1, ++it2)
    {
...
{
    for (auto key_value = other.values_.begin(); key_value != other.values_.end(); ++key_value)
    {
...
{
    if (message_.empty())
        message_ = "Attempting to " + std::string(operation_) + " the key \"" + DefaultKeyFormatter(key_) +
...
const char* ValuesIncorrectType::what() const noexcept
{
    if (message_.empty())
    {
        ...
        {
            message_ = "Attempting to retrieve value with key \"" + DefaultKeyFormatter(key_) +
                       "\", type stored in Values is " + storedTypeName + " but requested type was " +
                       requestedTypeName;
        }
    }
    return message_.c_str();
...
const char* NoMatchFoundForFixed::what() const noexcept
{
    ...
    {
        ostringstream oss;
        oss << "Attempting to retrieve fixed-size matrix with dimensions "                    //
            << M1_ << "x" << N1_ << ", but found dynamic Matrix with mismatched dimensions "  //
            << M2_ << "x" << N2_;
        message_ = oss.str();
    }
    return message_.c_str();

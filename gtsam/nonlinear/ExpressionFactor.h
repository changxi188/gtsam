...
public:
    ...
    bool equals(const NonlinearFactor& f, double tol) const override
    {
        ...
        return p && NoiseModelFactor::equals(f, tol) && traits<T>::Equals(measured_, p->measured_, tol) &&
               dims_ == p->dims_;
    }
    ...
    boost::shared_ptr<GaussianFactor> linearize(const Values& x) const override
    {
        ...
        {
            noiseModel = boost::static_pointer_cast<noiseModel::Constrained>(noiseModel_)->unit();
        }

        // Create a writeable JacobianFactor in advance
        boost::shared_ptr<JacobianFactor> factor(new JacobianFactor(keys_, dims_, Dim, noiseModel));
...
protected:
    ...
    void initialize(const Expression<T>& expression)
    {
        ...
        if (noiseModel_->dim() != Dim)
            throw std::invalid_argument("ExpressionFactor was created with a NoiseModel of incorrect dimension.");
...
public:
    ...
    {
        throw std::runtime_error("ExpressionFactorN::expression not provided: cannot deserialize.");
    }
...
private:
    ...
    {
        ar& boost::serialization::make_nvp("ExpressionFactorN",
                                           boost::serialization::base_object<ExpressionFactor<T>>(*this));
    }
};
/// traits
template <typename T, typename... Args>
struct traits<ExpressionFactorN<T, Args...>> : public Testable<ExpressionFactorN<T, Args...>>
{
};
// ExpressionFactorN
...
public:
    ...
    Vector evaluateError(const A1& a1, const A2& a2, boost::optional<Matrix&> H1 = boost::none,

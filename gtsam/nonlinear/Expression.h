...
public:
    ...
    {
        typedef std::function<T(const A1&, const A2&, typename MakeOptionalJacobian<T, A1>::type,
    ...
    {
        typedef std::function<T(const A1&, const A2&, const A3&, typename MakeOptionalJacobian<T, A1>::type,
                                typename MakeOptionalJacobian<T, A2>::type, typename MakeOptionalJacobian<T, A3>::type)>
            type;
    };
    ...
    Expression(typename BinaryFunction<A1, A2>::type function, const Expression<A1>& expression1,
               const Expression<A2>& expression2);

    /// Construct a ternary function expression
    template <typename A1, typename A2, typename A3>
    Expression(typename TernaryFunction<A1, A2, A3>::type function, const Expression<A1>& expression1,
...
protected:
    ...
    T valueAndDerivatives(const Values& values, const KeyVector& keys, const FastVector<int>& dims,
...
Expression<T> linearExpression(const std::function<T(A)>& f, const Expression<A>& expression,
                               const Eigen::Matrix<double, traits<T>::dimension, traits<A>::dimension>& dTdA)
{
    // Use lambda to endow f with a linear Jacobian
    typename Expression<T>::template UnaryFunction<A>::type g = [=](const A&                                  value,

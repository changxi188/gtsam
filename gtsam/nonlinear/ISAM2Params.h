...
{
    double wildfireThreshold;  ///< Continue updating the linear delta only when
                               ///< changes are above this threshold (default: 0.001)

    /** Specify parameters as constructor arguments */
    ISAM2GaussNewtonParams(double _wildfireThreshold = 0.001  ///< see ISAM2GaussNewtonParams public variables,
...
struct GTSAM_EXPORT ISAM2DoglegParams
{
    ...
    DoglegOptimizerImpl::TrustRegionAdaptationMode adaptationMode;  ///< See description in
                                                                    ///< DoglegOptimizerImpl::TrustRegionAdaptationMode
    bool verbose;  ///< Whether Dogleg prints iteration and convergence information

    /** Specify parameters as constructor arguments */
    ISAM2DoglegParams(double _initialDelta      = 1.0,   ///< see ISAM2DoglegParams::initialDelta
                      ...
                      DoglegOptimizerImpl::TrustRegionAdaptationMode _adaptationMode =
                          DoglegOptimizerImpl::SEARCH_EACH_ITERATION,  ///< see ISAM2DoglegParams::adaptationMode
...
{
    typedef boost::variant<ISAM2GaussNewtonParams, ISAM2DoglegParams> OptimizationParams;  ///< Either
                                                                                           ///< ISAM2GaussNewtonParams
                                                                                           ///< or ISAM2DoglegParams
    typedef boost::variant<double, FastMap<char, Vector> > RelinearizationThreshold;       ///< Either a constant
                                                                                      ///< relinearization threshold or
                                                                                      ///< a per-variable-type set of
                                                                                      ///< thresholds
    ...
    KeyFormatter keyFormatter;  ///< A KeyFormatter for when keys are printed during
    ...
    ISAM2Params(OptimizationParams       _optimizationParams   = ISAM2GaussNewtonParams(),
                ...
                Factorization _factorization = ISAM2Params::CHOLESKY, bool _cacheLinearizedFactors = true,
    ...
    void print(const std::string& str = "") const
    {
        ...
        else
        {
            ...
            {
                cout << "                                   '" << value.first << "' -> [" << value.second.transpose()
                     << " ]\n";
            }
        }
    ...
    {
        return factorization == CHOLESKY ? (GaussianFactorGraph::Eliminate)EliminatePreferCholesky :

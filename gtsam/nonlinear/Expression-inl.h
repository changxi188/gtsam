...
template <typename T>
Expression<T>::Expression(const T& value) : root_(new internal::ConstantExpression<T>(value))
{
}

template <typename T>
Expression<T>::Expression(const Key& key) : root_(new internal::LeafExpression<T>(key))
{
}

template <typename T>
Expression<T>::Expression(const Symbol& symbol) : root_(new internal::LeafExpression<T>(symbol))
{
}

template <typename T>
Expression<T>::Expression(unsigned char c, std::uint64_t j) : root_(new internal::LeafExpression<T>(Symbol(c, j)))
{
}
...
template <typename T>
template <typename A>
Expression<T>::Expression(typename UnaryFunction<A>::type function, const Expression<A>& expression)
...
Expression<T>::Expression(typename BinaryFunction<A1, A2>::type function, const Expression<A1>& expression1,
                          const Expression<A2>& expression2)
  : root_(new internal::BinaryExpression<T, A1, A2>(function, expression1, expression2))
{
}
...
Expression<T>::Expression(typename TernaryFunction<A1, A2, A3>::type function, const Expression<A1>& expression1,
                          const Expression<A2>& expression2, const Expression<A3>& expression3)
  : root_(new internal::TernaryExpression<T, A1, A2, A3>(function, expression1, expression2, expression3))
{
}
...
Expression<T>::Expression(const Expression<A>& expression,
  ...
  : root_(new internal::UnaryExpression<T, A>(std::bind(method, std::placeholders::_1, std::placeholders::_2),
                                              expression))
{
}
...
Expression<T>::Expression(const Expression<A1>& expression1,
                          ...
                          const Expression<A2>& expression2)
  : root_(new internal::BinaryExpression<T, A1, A2>(
        std::bind(method, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4),
...
template <typename A1, typename A2, typename A3>
Expression<T>::Expression(const Expression<A1>& expression1,
                          T (A1::*method)(const A2&, const A3&, typename MakeOptionalJacobian<T, A1>::type,
                                          typename MakeOptionalJacobian<T, A2>::type,
                                          typename MakeOptionalJacobian<T, A3>::type) const,
                          const Expression<A2>& expression2, const Expression<A3>& expression3)
  : root_(new internal::TernaryExpression<T, A1, A2, A3>(std::bind(method, std::placeholders::_1, std::placeholders::_2,
...
template <typename T>
T Expression<T>::valueAndDerivatives(const Values& values, const KeyVector& keys, const FastVector<int>& dims,
...
template <typename T>
T Expression<T>::traceExecution(const Values& values, internal::ExecutionTrace<T>& trace, void* traceStorage) const
{
    return root_->traceExecution(values, trace, static_cast<internal::ExecutionTraceStorage*>(traceStorage));
}

template <typename T>
T Expression<T>::valueAndJacobianMap(const Values& values, internal::JacobianMap& jacobians) const
{
...
#ifdef _MSC_VER
    std::unique_ptr<void, void (*)(void*)> traceStorageDeleter(_aligned_malloc(size, internal::TraceAlignment),
...
struct apply_compose
{
    ...
    static const int Dim = traits<T>::dimension;
    T                operator()(const T& x, const T& y, OptionalJacobian<Dim, Dim> H1 = boost::none,
                 OptionalJacobian<Dim, Dim> H2 = boost::none) const
    {
...
{
    double operator()(const double& x, const double& y, OptionalJacobian<1, 1> H1 = boost::none,
...
{
    return Expression<T>(std::bind(internal::apply_compose<T>(), std::placeholders::_1, std::placeholders::_2,
                                   std::placeholders::_3, std::placeholders::_4),
                         expression1, expression2);
}

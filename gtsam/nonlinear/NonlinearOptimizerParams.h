...
public:
    ...
    Ordering::OrderingType orderingType =
        Ordering::COLAMD;  ///< The method of ordering use during variable elimination (default COLAMD)
    ...
    using IterationHook = std::function<void(size_t /*iteration*/, double /*errorBefore*/, double /*errorAfter*/)>;
    ...
    LinearSolverType linearSolverType =
        MULTIFRONTAL_CHOLESKY;           ///< The type of linear solver to use in the nonlinear optimizer
    boost::optional<Ordering> ordering;  ///< The optional variable elimination ordering, or empty to use COLAMD
                                         ///< (default: empty)
    IterativeOptimizationParameters::shared_ptr iterativeParams;  ///< The container for iterativeOptimization
                                                                  ///< parameters. used in CG Solvers.
    ...
    // Note that if you want to use a custom ordering, you must set the ordering directly, this will switch to custom
    // type

...
namespace internal
{
inline static void optimizeInPlace(const ISAM2::sharedClique& clique, VectorValues* result)
{
...
/* ************************************************************************* */
size_t DeltaImpl::UpdateGaussNewtonDelta(const ISAM2::Roots& roots, const KeySet& replacedKeys,
    ...
    else
    {
        ...
        for (const ISAM2::sharedClique& root : roots)
            lastBacksubVariableCount +=
                optimizeWildfireNonRecursive(root, wildfireThreshold, replacedKeys, delta);  // modifies delta
...
namespace internal
{
void updateRgProd(const ISAM2::sharedClique& clique, const KeySet& replacedKeys, const VectorValues& grad,
                  VectorValues* RgProd, size_t* varsUpdated)
{
    ...
    if (anyReplaced)
    {
        ...
        Vector gR =
            grad.vector(KeyVector(clique->conditional()->beginFrontals(), clique->conditional()->endFrontals()));
...
/* ************************************************************************* */
size_t DeltaImpl::UpdateRgProd(const ISAM2::Roots& roots, const KeySet& replacedKeys, const VectorValues& gradAtZero,
                               VectorValues* RgProd)
{
    // Update variables

...
public:
    ...
    void optimizeWildfire(const KeySet& replaced, double threshold, KeySet* changed, VectorValues* delta,
                          size_t* count) const;

    bool optimizeWildfireNode(const KeySet& replaced, double threshold, KeySet* changed, VectorValues* delta,
                              size_t* count) const;
...
private:
     ...
     * clique was replaced.
     */
    bool valuesChanged(const KeySet& replaced, const Vector& originalValues, const VectorValues& delta,
 ...
 */
size_t optimizeWildfire(const ISAM2Clique::shared_ptr& root, double threshold, const KeySet& replaced,
                        VectorValues* delta);

size_t optimizeWildfireNonRecursive(const ISAM2Clique::shared_ptr& root, double threshold, const KeySet& replaced,
                                    VectorValues* delta);

...
struct GTSAM_EXPORT DeltaImpl
{
     ...
     */
    static size_t UpdateGaussNewtonDelta(const ISAM2::Roots& roots, const KeySet& replacedKeys,
     ...
     */
    static size_t UpdateRgProd(const ISAM2::Roots& roots, const KeySet& replacedKeys, const VectorValues& gradAtZero,
                               VectorValues* RgProd);
...
struct GTSAM_EXPORT UpdateImpl
{
    ...
    bool relinarizationNeeded(size_t update_count) const
    {
        return updateParams_.force_relinearize ||
               (params_.enableRelinearization && update_count % params_.relinearizeSkip == 0);
    }

    // Add any new factors \Factors:=\Factors\cup\Factors'.
    void pushBackFactors(const NonlinearFactorGraph& newFactors, NonlinearFactorGraph* nonlinearFactors,
                         GaussianFactorGraph* linearFactors, VariableIndex* variableIndex,
                         FactorIndices* newFactorsIndices, KeySet* keysWithRemovedFactors) const
    {
        gttic(pushBackFactors);
        ...
        *newFactorsIndices = nonlinearFactors->add_factors(newFactors, params_.findUnusedFactorSlots);
        ...
        for (const auto index : updateParams_.removeFactorIndices)
        {
            ...
            if (params_.cacheLinearizedFactors)
                linearFactors->remove(index);
        }

        // Remove removed factors from the variable index so we do not attempt to
        // relinearize them
        variableIndex->remove(updateParams_.removeFactorIndices.begin(), updateParams_.removeFactorIndices.end(),
                              removedFactors);
    ...
    void computeUnusedKeys(const NonlinearFactorGraph& newFactors, const VariableIndex& variableIndex,
                           const KeySet& keysWithRemovedFactors, KeySet* unusedKeys) const
    {
        ...
        std::set_difference(removedAndEmpty.begin(), removedAndEmpty.end(), newFactorSymbKeys.begin(),
                            newFactorSymbKeys.end(), std::inserter(*unusedKeys, unusedKeys->end()));
    }

    // Calculate nonlinear error
    void error(const NonlinearFactorGraph& nonlinearFactors, const Values& estimate,
    ...
    // Mark linear update
    void gatherInvolvedKeys(const NonlinearFactorGraph& newFactors, const NonlinearFactorGraph& nonlinearFactors,
                            const KeySet& keysWithRemovedFactors, KeySet* markedKeys) const
    {
    ...
    static void CheckRelinearizationRecursiveMap(const FastMap<char, Vector>& thresholds, const VectorValues& delta,
                                                 const ISAM2::sharedClique& clique, KeySet* relinKeys)
    {
        ...
        for (Key var : *clique->conditional())
        {
            ...
            if (threshold.rows() != deltaVar.rows())
                throw std::invalid_argument("Relinearization threshold vector dimensionality for '" +
    ...
    static void CheckRelinearizationRecursiveDouble(double threshold, const VectorValues& delta,
                                                    const ISAM2::sharedClique& clique, KeySet* relinKeys)
    {
        // Check the current clique for relinearization
        bool relinearize = false;
     ...
     */
    static KeySet CheckRelinearizationPartial(const ISAM2::Roots& roots, const VectorValues& delta,
                                              const ISAM2Params::RelinearizationThreshold& relinearizeThreshold)
    {
        ...
        for (const ISAM2::sharedClique& root : roots)
        {
            if (relinearizeThreshold.type() == typeid(double))
                ...
                CheckRelinearizationRecursiveMap(boost::get<FastMap<char, Vector> >(relinearizeThreshold), delta, root,
                                                 &relinKeys);
        }
        return relinKeys;
    }
    ...
    KeySet gatherRelinearizeKeys(const ISAM2::Roots& roots, const VectorValues& delta, const KeySet& fixedVariables,
                                 KeySet* markedKeys) const
    {
        gttic(gatherRelinearizeKeys);
        // J=\{\Delta_{j}\in\Delta|\Delta_{j}\geq\beta\}.
        KeySet relinKeys = params_.enablePartialRelinearizationCheck ?
                               CheckRelinearizationPartial(roots, delta, params_.relinearizeThreshold) :
    ...
    // their ancestors.
    void findFluid(const ISAM2::Roots& roots, const KeySet& relinKeys, KeySet* markedKeys,
    ...
    void linearizeNewFactors(const NonlinearFactorGraph& newFactors, const Values& theta, size_t numNonlinearFactors,
                             const FactorIndices& newFactorsIndices, GaussianFactorGraph* linearFactors) const
    {
    ...
    void augmentVariableIndex(const NonlinearFactorGraph& newFactors, const FactorIndices& newFactorsIndices,

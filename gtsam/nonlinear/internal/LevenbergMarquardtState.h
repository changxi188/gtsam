...
struct LevenbergMarquardtState : public NonlinearOptimizerState
{
    ...
    LevenbergMarquardtState(const Values& initialValues, double error, double lambda, double currentFactor,
    ...
    std::unique_ptr<This> decreaseLambda(const LevenbergMarquardtParams& params, double stepQuality, Values&& newValues,
                                         double newError) const
    {
        ...
        return std::unique_ptr<This>(new This(std::move(newValues), newError, newLambda, newFactor, iterations + 1,
                                              totalNumberInnerIterations + 1));
    }
    ...
    struct CachedModel
    {
        CachedModel()
        {
        }  // default int makes zero-size matrices
        CachedModel(int dim, double sigma)
          : A(Matrix::Identity(dim, dim)), b(Vector::Zero(dim)), model(noiseModel::Isotropic::Sigma(dim, sigma))
        {
        }

...
public:
    ...
    NonlinearFactor(const CONTAINER& keys) : Base(keys)
    {
    }
...
/// traits
template <>
struct traits<NonlinearFactor> : public Testable<NonlinearFactor>
{
};
...
public:
     ...
     */
    template <typename CONTAINER>
    NoiseModelFactor(const SharedNoiseModel& noiseModel, const CONTAINER& keys) : Base(keys), noiseModel_(noiseModel)
    {
    }
...
private:
    ...
    {
        ar& boost::serialization::make_nvp("NonlinearFactor", boost::serialization::base_object<Base>(*this));
...
template <typename T1, typename T2, typename T3, typename T4, typename T5, typename T6, typename... TExtra>
...
class NoiseModelFactorN : public NoiseModelFactor, public detail::NoiseModelFactorAliases<ValueTypes...>
{
public:
...
public:
    ...
    NoiseModelFactorN(const SharedNoiseModel& noiseModel, CONTAINER keys) : Base(noiseModel, keys)
    {
        if (keys.size() != N)
        {
            throw std::invalid_argument("NoiseModelFactorN: wrong number of keys given");
        }
    }
...
private:
    ...
    inline Vector unwhitenedError(boost::mp11::index_sequence<Indices...>,  //
                                  const Values& x, boost::optional<std::vector<Matrix>&> H = boost::none) const
    {
        if (this->active(x))
        {
            if (H)
            {
                return evaluateError(x.at<ValueTypes>(keys_[Indices])..., (*H)[Indices]...);
            }
            else
            {
    ...
    void serialize(ARCHIVE& ar, const unsigned int /*version*/)
    {
        ar& boost::serialization::make_nvp("NoiseModelFactor", boost::serialization::base_object<Base>(*this));
    }

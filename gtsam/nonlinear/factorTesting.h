 ...
 */
inline JacobianFactor linearizeNumerically(const NoiseModelFactor& factor, const Values& values, double delta = 1e-5)
{
...
inline bool testFactorJacobians(const std::string& name_, const NoiseModelFactor& factor, const gtsam::Values& values,
                                double delta, double tolerance)
{
    ...
    auto actual = boost::dynamic_pointer_cast<JacobianFactor>(factor.linearize(values));
    if (!actual)
        return false;

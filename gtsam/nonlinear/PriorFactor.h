...
public:
    ...
    /** Constructor */
    PriorFactor(Key key, const VALUE& prior, const SharedNoiseModel& model = nullptr) : Base(model, key), prior_(prior)
    {
    }
    ...
    {
        if (H)
            (*H) = Matrix::Identity(traits<T>::GetDimension(x), traits<T>::GetDimension(x));

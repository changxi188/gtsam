...
public:
    ...
    ConstantBias(const Vector3& biasAcc, const Vector3& biasGyro) : biasAcc_(biasAcc), biasGyro_(biasGyro)
    {
    }

    explicit ConstantBias(const Vector6& v) : biasAcc_(v.head<3>()), biasGyro_(v.tail<3>())
    {
    }
    ...
    Vector3 correctAccelerometer(const Vector3& measurement, OptionalJacobian<3, 6> H1 = boost::none,
    ...
    Vector3 correctGyroscope(const Vector3& measurement, OptionalJacobian<3, 6> H1 = boost::none,
                             OptionalJacobian<3, 3> H2 = boost::none) const
    {
        if (H1)
            (*H1) << Z_3x3, -I_3x3;
    ...
    /// @name Deprecated
    /// @{
    ConstantBias GTSAM_DEPRECATED inverse()
    {
        return -(*this);
    }
...
template <>
struct traits<imuBias::ConstantBias> : public internal::VectorSpace<imuBias::ConstantBias>
{
};

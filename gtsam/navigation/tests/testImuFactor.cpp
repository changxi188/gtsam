...
/* ************************************************************************* */
Rot3 evaluateRotationError(const ImuFactor& factor, const Pose3& pose_i, const Vector3& vel_i, const Pose3& pose_j,
                           const Vector3& vel_j, const Bias& bias)
{
    return Rot3::Expmap(factor.evaluateError(pose_i, vel_i, pose_j, vel_j, bias).head(3));
}
...
TEST(ImuFactor, Accelerating)
{
    ...
    const AcceleratingScenario scenario(nRb, initial_position, initial_velocity, Vector3(a, 0, 0));
...
TEST(ImuFactor, PreintegratedMeasurements)
{
...

    {
        ...
        std::function<Vector9(const NavState&, const NavState&, const Bias&)> f =
            std::bind(&PreintegrationBase::computeError, actual, std::placeholders::_1, std::placeholders::_2,
    ...
    Vector3 expectedDeltaV2 = Vector3(0.05, 0.0, 0.0) + Rot3::Expmap(expectedDeltaR1) * measuredAcc * 0.5;
...
TEST(ImuFactor, PreintegrationBaseMethods)
{
    ...
    Matrix expectedH = numericalDerivative11<Vector9, Bias>(
        std::bind(&PreintegrationBase::biasCorrectedDelta, pim, std::placeholders::_1, boost::none), kZeroBias);
    ...
    Matrix   eH1            = numericalDerivative11<NavState, NavState>(
        std::bind(&PreintegrationBase::predict, pim, std::placeholders::_1, kZeroBias, boost::none, boost::none),
        state1);
    EXPECT(assert_equal(eH1, aH1));
    Matrix eH2 = numericalDerivative11<NavState, Bias>(
        std::bind(&PreintegrationBase::predict, pim, state1, std::placeholders::_1, boost::none, boost::none),
        kZeroBias);
...
TEST(ImuFactor, ErrorAndJacobians)
{
    ...
    EXPECT(assert_equal(expectedError, factor.evaluateError(x1, v1, x2, v2, kZeroBias)));
    ...
    Matrix H1Rot3 = numericalDerivative11<Rot3, Pose3>(
        std::bind(&evaluateRotationError, factor, std::placeholders::_1, v1, x2, v2, kZeroBias), x1);
    EXPECT(assert_equal(H1Rot3, H1a.topRows(3)));

    Matrix H3Rot3 = numericalDerivative11<Rot3, Pose3>(
        std::bind(&evaluateRotationError, factor, x1, v1, std::placeholders::_1, v2, kZeroBias), x2);
    ...
    EXPECT(assert_equal(expectedError, factor.evaluateError(x1, v1, x2, v2_wrong, kZeroBias), 1e-2));
...
TEST(ImuFactor, ErrorAndJacobianWithBiases)
{
    ...
    Vector3 measuredAcc = x1.rotation().unrotate(-kGravityAlongNavZDown) + Vector3(0.2, 0.0, 0.0);
    double  deltaT      = 1.0;
    ...
    Matrix expectedH = numericalDerivative11<Vector9, Bias>(
        std::bind(&PreintegrationBase::biasCorrectedDelta, pim, std::placeholders::_1, boost::none), bias);
    EXPECT(assert_equal(expectedH, actualH));
...
TEST(ImuFactor, ErrorAndJacobianWith2ndOrderCoriolis)
{
    ...
    PreintegratedImuMeasurements pim(p, Bias(Vector3(0.2, 0.0, 0.0), Vector3(0.0, 0.0, 0.1)));
    pim.integrateMeasurement(measuredAcc, measuredOmega, deltaT);
...
TEST(ImuFactor, PartialDerivative_wrt_Bias)
{
    ...
    auto evaluateRotation = [=](const Vector3 biasOmega) { return Rot3::Expmap((measuredOmega - biasOmega) * deltaT); };

    // Compute numerical derivatives
    Matrix expectedDelRdelBiasOmega = numericalDerivative11<Rot3, Vector3>(evaluateRotation, biasOmega);
...
TEST(ImuFactor, PartialDerivativeLogmap)
{
    ...
    // Compute numerical derivatives
    Matrix expectedDelFdeltheta = numericalDerivative11<Vector, Vector3>(evaluateLogRotation, deltatheta);
...
TEST(ImuFactor, ErrorWithBiasesAndSensorBodyDisplacement)
{
    ...
    const AcceleratingScenario scenario(nRb, p1, v1, a, Vector3(0, 0, M_PI / 10.0 + 0.3));
    ...
    pim.correctMeasurementsBySensorPose(measuredAcc, measuredOmega, boost::none, D_correctedAcc_measuredOmega,
                                        boost::none);
    Matrix3 expectedD = numericalDerivative11<Vector3, Vector3>(
        std::bind(correctedAcc, pim, measuredAcc, std::placeholders::_1), measuredOmega, 1e-6);
...
TEST(ImuFactor, PredictPositionAndVelocity)
{
    ...
    PreintegratedImuMeasurements pim(testing::Params(), Bias(Vector3(0.2, 0.0, 0.0), Vector3(0.0, 0.0, 0.0)));
...
TEST(ImuFactor, PredictArbitrary)
{
    ...
    const AcceleratingScenario scenario(x1.rotation(), x1.translation(), v1, Vector3(0.1, 0.2, 0),
...
TEST(ImuFactor, bodyPSensorWithBias)
{
    ...
    noiseBetweenBiasSigma << Vector3(2.0e-5, 2.0e-5, 2.0e-5), Vector3(3.0e-6, 3.0e-6, 3.0e-6);
    ...
    {
        PreintegratedImuMeasurements pim = PreintegratedImuMeasurements(p, priorBias);
        for (int j = 0; j < 200; ++j)
            pim.integrateMeasurement(measuredAcc, measuredOmega, deltaT);
...
struct ImuFactorMergeTest
{
    ...
    int TestScenario(TestResult& result_, const std::string& name_, const Scenario& scenario, const Bias& bias01,
                     const Bias& bias12, double tol)
    {
        ...
        EXPECT(assert_equal(pim02_expected.preintegrated(), actual_pim02.preintegrated(), tol));
        EXPECT(assert_equal(pim02_expected, actual_pim02, tol));

        ImuFactor::shared_ptr factor01 = boost::make_shared<ImuFactor>(X(0), V(0), X(1), V(1), B(0), pim01);
        ImuFactor::shared_ptr factor12 = boost::make_shared<ImuFactor>(X(1), V(1), X(2), V(2), B(0), pim12);
        ImuFactor::shared_ptr factor02_expected =
            boost::make_shared<ImuFactor>(X(0), V(0), X(2), V(2), B(0), pim02_expected);
    ...
    void TestScenarios(TestResult& result_, const std::string& name_, const Bias& bias01, const Bias& bias12,
                       double tol)
    {
...
{
    ImuFactorMergeTest mergeTest;
    mergeTest.TestScenarios(result_, name_, Bias(Vector3(0.03, -0.02, 0.01), Vector3(-0.01, 0.02, -0.03)),

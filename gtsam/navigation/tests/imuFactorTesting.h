...
{
    ImuMeasurement(const Vector3& acc, const Vector3& gyro, double dt) : acc(acc), gyro(gyro), dt(dt)
    {
    }
...
struct SomeMeasurements : vector<ImuMeasurement>
{
    SomeMeasurements()
    {
        ...
        for (int i = 1; i < 100; i++)
        {
            emplace_back(Vector3(0.05, 0.09, 0.01), Vector3(pi100, pi100 * 3, 2 * pi100), dt);
        }
    }
};

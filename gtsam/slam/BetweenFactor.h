...
public:
    ...
    {
        std::cout << s << "BetweenFactor(" << keyFormatter(this->key1()) << "," << keyFormatter(this->key2()) << ")\n";
    ...
    Vector evaluateError(const T& p1, const T& p2, boost::optional<Matrix&> H1 = boost::none,
...
private:
    ...
    enum
    {
        NeedsToAlign = (sizeof(VALUE) % 16) == 0
    };

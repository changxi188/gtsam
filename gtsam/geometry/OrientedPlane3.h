...
public:
    ...
    OrientedPlane3(const Unit3& n, double d) : n_(n), d_(d)
    {
    }

    /// Construct from a vector of plane coefficients
    explicit OrientedPlane3(const Vector4& vec) : n_(vec(0), vec(1), vec(2)), d_(vec(3))
    {
    }
     ...
     */
    OrientedPlane3 transform(const Pose3& xr, OptionalJacobian<3, 3> Hp = boost::none,
...
template <>
struct traits<OrientedPlane3> : public internal::Manifold<OrientedPlane3>
{
};

template <>
struct traits<const OrientedPlane3> : public internal::Manifold<OrientedPlane3>
{
};

 ...
 */
GTSAM_EXPORT Vector4
triangulateHomogeneousDLT(const std::vector<Matrix34, Eigen::aligned_allocator<Matrix34>>& projection_matrices,
 ...
 */
GTSAM_EXPORT Point3 triangulateDLT(const std::vector<Matrix34, Eigen::aligned_allocator<Matrix34>>& projection_matrices,
                                   const Point2Vector& measurements, double rank_tol = 1e-9);
 ...
 */
GTSAM_EXPORT Point3 triangulateLOST(const std::vector<Pose3>& poses, const Point3Vector& calibratedMeasurements,
...
std::pair<NonlinearFactorGraph, Values> triangulationGraph(const CameraSet<CAMERA>&                  cameras,
                                                           ...
                                                           const SharedNoiseModel& model = nullptr)
{
    ...
    values.insert(landmarkKey, initialEstimate);  // Initial landmark value
    NonlinearFactorGraph    graph;
    static SharedNoiseModel unit(noiseModel::Unit::Create(traits<typename CAMERA::Measurement>::dimension));
    for (size_t i = 0; i < measurements.size(); i++)
    {
 ...
 */
template <class CALIBRATION>
Point3 triangulateNonlinear(const std::vector<Pose3>& poses, boost::shared_ptr<CALIBRATION> sharedCal,
 ...
 */
template <class CAMERA>
Point3 triangulateNonlinear(const CameraSet<CAMERA>& cameras, const typename CAMERA::MeasurementVector& measurements,
...
template <class CALIBRATION>
std::vector<Matrix34, Eigen::aligned_allocator<Matrix34>>
projectionMatricesFromPoses(const std::vector<Pose3>& poses, boost::shared_ptr<CALIBRATION> sharedCal)
{
    std::vector<Matrix34, Eigen::aligned_allocator<Matrix34>> projection_matrices;
...
MEASUREMENT undistortMeasurementInternal(const CALIBRATION& cal, const MEASUREMENT& measurement,
...
inline PinholeCamera<Cal3_S2>::MeasurementVector undistortMeasurements(
    const CameraSet<PinholeCamera<Cal3_S2>>& cameras, const PinholeCamera<Cal3_S2>::MeasurementVector& measurements)
{
    return measurements;
}
...
inline SphericalCamera::MeasurementVector undistortMeasurements(const CameraSet<SphericalCamera>&         cameras,
...
inline Point3Vector calibrateMeasurementsShared(const CALIBRATION& cal, const Point2Vector& measurements)
{
    ...
    // Calibrate with cal and uncalibrate with pinhole version of cal so that
    // measurements are undistorted.
    std::transform(measurements.begin(), measurements.end(), std::back_inserter(calibratedMeasurements),
...
Point3 triangulatePoint3(const std::vector<Pose3>& poses, boost::shared_ptr<CALIBRATION> sharedCal,
                         const Point2Vector& measurements, double rank_tol = 1e-9, bool optimize = false,
                         const SharedNoiseModel& model = nullptr, const bool useLOST = false)
{
    ...
    if (useLOST)
    {
        ...
        // plane.
        auto calibratedMeasurements = calibrateMeasurementsShared<CALIBRATION>(*sharedCal, measurements);
 ...
 */
template <class CAMERA>
Point3 triangulatePoint3(const CameraSet<CAMERA>& cameras, const typename CAMERA::MeasurementVector& measurements,
                         double rank_tol = 1e-9, bool optimize = false, const SharedNoiseModel& model = nullptr,
...
template <class CALIBRATION>
Point3 triangulatePoint3(const CameraSet<PinholeCamera<CALIBRATION>>& cameras, const Point2Vector& measurements,
                         double rank_tol = 1e-9, bool optimize = false, const SharedNoiseModel& model = nullptr,
                         const bool useLOST = false)
{
...
struct GTSAM_EXPORT TriangulationParameters
{
     ...
     *
     */
    TriangulationParameters(const double _rankTolerance = 1.0, const bool _enableEPI = false,
...
public:
     ...
     * Constructor
     */
    TriangulationResult(const Point3& p) : status(VALID)
    {
        reset(p);
    }
                                    ...
                                    const TriangulationParameters&            params)
{
    ...
    if (m < 2)
        ...
        {
            Point3 point =
                triangulatePoint3<CAMERA>(cameras, measured, params.rankTolerance, params.enableEPI, params.noiseModel);
        ...
        catch (TriangulationUnderconstrainedException&)
        {
            ...
            // 1) There is a single pose for triangulation - this should not happen because we checked the number of
            // poses before 2) The rank of the matrix used for triangulation is < 3: rotation-only, parallel cameras (or
            // motion towards the landmark)
            return TriangulationResult::Degenerate();
        }
        catch (TriangulationCheiralityException&)
        {
            // point is behind one of the cameras: can be the case of close-to-parallel cameras or may depend on
            // outliers
            return TriangulationResult::BehindCamera();
        }
}

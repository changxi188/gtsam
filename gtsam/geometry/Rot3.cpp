...
/* ************************************************************************* */
Unit3 Rot3::rotate(const Unit3& p, OptionalJacobian<2, 3> HR, OptionalJacobian<2, 2> Hp) const
{
    Matrix32 Dp;
...
/* ************************************************************************* */
Unit3 Rot3::unrotate(const Unit3& p, OptionalJacobian<2, 3> HR, OptionalJacobian<2, 2> Hp) const
{
    Matrix32 Dp;
...
Point3 Rot3::unrotate(const Point3& p, OptionalJacobian<3, 3> H1, OptionalJacobian<3, 3> H2) const
{
...
Vector3 Rot3::ypr(OptionalJacobian<3, 3> H) const
{
    Vector3 q = xyz(H);
    if (H)
        H->row(0).swap(H->row(2));

    return Vector3(q(2), q(1), q(0));
}

/* ************************************************************************* */
Vector3 Rot3::rpy(OptionalJacobian<3, 3> H) const
{
    return xyz(H);
}
...
pair<Matrix3, Vector3> RQ(const Matrix3& A, OptionalJacobian<3, 9> H)
{
    ...
    if (H)
    {
        if (std::abs(y - M_PI / 2) < 1e-2)
            throw std::runtime_error("Rot3::RQ : Derivative undefined at singularity (gimbal lock)");
        ...
        *H = Matrix39::Zero();
        ...
        H->block<1, 9>(2, 0) = atan_d1(C(1, 0), C(1, 1)) * c10HA + atan_d2(C(1, 0), C(1, 1)) * c11HA;
    }

...
/* ************************************************************************* */
Pose3::Pose3(const Pose2& pose2) : R_(Rot3::Rodrigues(0, 0, pose2.theta())), t_(Point3(pose2.x(), pose2.y(), 0))
{
}
...
Vector6 Pose3::Adjoint(const Vector6& xi_b, OptionalJacobian<6, 6> H_pose, OptionalJacobian<6, 6> H_xib) const
{
...
Vector6 Pose3::AdjointTranspose(const Vector6& x, OptionalJacobian<6, 6> H_pose, OptionalJacobian<6, 6> H_x) const
{
    const Matrix6 Ad   = AdjointMap();
    ...
    if (H_pose)
    {
        const auto w_T_hat = skewSymmetric(AdTx.head<3>()), v_T_hat = skewSymmetric(AdTx.tail<3>());
...
/* ************************************************************************* */
Vector6 Pose3::adjoint(const Vector6& xi, const Vector6& y, OptionalJacobian<6, 6> Hxi, OptionalJacobian<6, 6> H_y)
{
    if (Hxi)
    {
        Hxi->setZero();
...
/* ************************************************************************* */
Vector6 Pose3::adjointTranspose(const Vector6& xi, const Vector6& y, OptionalJacobian<6, 6> Hxi,
...
{
    return Pose3(interpolate<Rot3>(R_, T.R_, t), interpolate<Point3>(t_, T.t_, t));
}
...
Matrix3 Pose3::ComputeQforExpmapDerivative(const Vector6& xi, double nearZeroThreshold)
{
    ...
    if (std::abs(phi) > nearZeroThreshold)
    {
        ...
        Q = -0.5 * V + (phi - s) / phi3 * (W * V + V * W - WVW) +
            ...
            0.5 * ((1 - phi2 / 2 - c) / phi4 - 3 * (phi - s - phi3 / 6.) / phi5) * (WVW * W + W * WVW);
    }
    else
    {
        Q = -0.5 * V + 1. / 6. * (W * V + V * W - WVW) - 1. / 24. * (W * W * V + V * W * W - 3 * WVW) +
            1. / 120. * (WVW * W + W * WVW);
    }
#endif
...
/* ************************************************************************* */
Pose3 Pose3::transformPoseFrom(const Pose3& aTb, OptionalJacobian<6, 6> Hself, OptionalJacobian<6, 6> HaTb) const
{
...
/* ************************************************************************* */
Pose3 Pose3::transformPoseTo(const Pose3& wTb, OptionalJacobian<6, 6> Hself, OptionalJacobian<6, 6> HwTb) const
{
    if (Hself)
...
/* ************************************************************************* */
Point3 Pose3::transformFrom(const Point3& point, OptionalJacobian<3, 6> Hself, OptionalJacobian<3, 3> Hpoint) const
{
...
/* ************************************************************************* */
Point3 Pose3::transformTo(const Point3& point, OptionalJacobian<3, 6> Hself, OptionalJacobian<3, 3> Hpoint) const
{
    ...
    if (Hself)
    {
        ...
        (*Hself) << 0.0, -wz, +wy, -1.0, 0.0, 0.0, +wz, 0.0, -wx, 0.0, -1.0, 0.0, -wy, +wx, 0.0, 0.0, 0.0, -1.0;
    }
    if (Hpoint)
    {
        *Hpoint = Rt;
...
/* ************************************************************************* */
double Pose3::range(const Point3& point, OptionalJacobian<1, 6> Hself, OptionalJacobian<1, 3> Hpoint) const
{
...
/* ************************************************************************* */
double Pose3::range(const Pose3& pose, OptionalJacobian<1, 6> Hself, OptionalJacobian<1, 6> Hpose) const
{
...
/* ************************************************************************* */
Unit3 Pose3::bearing(const Point3& point, OptionalJacobian<2, 6> Hself, OptionalJacobian<2, 3> Hpoint) const
{
    Matrix36 D_local_pose;
    Matrix3  D_local_point;
    Point3   local = transformTo(point, Hself ? &D_local_pose : 0, Hpoint ? &D_local_point : 0);
...
/* ************************************************************************* */
Unit3 Pose3::bearing(const Pose3& pose, OptionalJacobian<2, 6> Hself, OptionalJacobian<2, 6> Hpose) const
{
    if (Hpose)
    {
        Hpose->setZero();
...
boost::optional<Pose3> Pose3::Align(const Matrix& a, const Matrix& b)
{
    ...
    {
        throw std::invalid_argument("Pose3:Align expects 3*N matrices of equal shape.");
    }

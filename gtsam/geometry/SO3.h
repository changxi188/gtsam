...
template <>
GTSAM_EXPORT Matrix3 SO3::Hat(const Vector3& xi);  ///< make skew symmetric matrix

template <>
GTSAM_EXPORT Vector3 SO3::Vee(const Matrix3& X);  ///< inverse of Hat
 ...
 */
template <>
GTSAM_EXPORT Vector3 SO3::Logmap(const SO3& R, ChartJacobian H);
 ...
 */
GTSAM_EXPORT Matrix3 compose(const Matrix3& M, const SO3& R, OptionalJacobian<9, 9> H = boost::none);
...
public:
    ...
    GTSAM_EXPORT Vector3 applyInvDexp(const Vector3& v, OptionalJacobian<3, 3> H1 = boost::none,
                                      OptionalJacobian<3, 3> H2 = boost::none) const;
};
}  //  namespace so3

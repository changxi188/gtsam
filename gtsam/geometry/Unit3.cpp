...
Unit3::Unit3(double x, double y, double z) : p_(x, y, z)
{
    p_.normalize();
}
...
Unit3 Unit3::retract(const Vector2& v, OptionalJacobian<2, 2> H) const
{
    ...
    {
        const Unit3 exp_p_xi_hat = Unit3::FromPoint3(c * p_ + xi_hat, H ? &H_from_point : nullptr);
        if (H)
        {  // Jacobian
            *H = H_from_point * (-p_ * xi_hat.transpose() + Matrix33::Identity()) * basis();
        }
        return exp_p_xi_hat;
    }

    const double st           = std::sin(theta) / theta;
    const Unit3  exp_p_xi_hat = Unit3::FromPoint3(c * p_ + xi_hat * st, H ? &H_from_point : nullptr);
    if (H)
    {  // Jacobian
        *H = H_from_point *

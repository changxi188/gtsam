...
{
    if (H)
        throw std::runtime_error("SO<N>::Retract jacobian not implemented.");
...
template <int N>
typename SO<N>::TangentVector SO<N>::ChartAtOrigin::Local(const SO& R, ChartJacobian H)
{
    if (H)
        throw std::runtime_error("SO<N>::Local jacobian not implemented.");
...
{
    if (N == 2)
        return I_1x1;  // SO(2) case
    throw std::runtime_error("SO<N>::AdjointMap only implemented for SO2, SO3 and SO4.");
}
...
template <int N>
typename SO<N>::VectorN2 SO<N>::vec(OptionalJacobian<internal::NSquaredSO(N), dimension> H) const
{
    // Vectorize
    ...
    if (H)
    {
        ...
        {
            H->block(i * N, 0, N, dimension) = matrix_ * P.block(i * N, 0, N, dimension);
        }
    }
    return X;
}

...
public:
    ...
    explicit PinholeCamera(const Pose3& pose) : Base(pose)
    {
    }

    /** constructor with pose and calibration */
    PinholeCamera(const Pose3& pose, const Calibration& K) : Base(pose), K_(K)
    {
    }
     ...
     */
    static PinholeCamera Lookat(const Point3& eye, const Point3& target, const Point3& upVector,
    ...
    explicit PinholeCamera(const Vector& v) : Base(v.head<6>())
    {
        if (v.size() > 6)
    ...
    PinholeCamera(const Vector& v, const Vector& K) : Base(v), K_(K)
    {
    }
    ...
    PinholeCamera retract(const Vector& d) const
    {
        if ((size_t)d.size() == 6)
            ...
            return PinholeCamera(this->pose().retract(d.head<6>()), calibration().retract(d.tail(calibration().dim())));
    }
    ...
    Point2 project2(const Point3& pw, OptionalJacobian<2, dimension> Dcamera = boost::none,
    ...
    Point2 project2(const Unit3& pw, OptionalJacobian<2, dimension> Dcamera = boost::none,
     ...
     * @return range (double)
     */
    double range(const Point3& point, OptionalJacobian<1, dimension> Dcamera = boost::none,
     ...
     * @return range (double)
     */
    double range(const Pose3& pose, OptionalJacobian<1, dimension> Dcamera = boost::none,
    ...
    double range(const PinholeCamera<CalibrationB>& camera, OptionalJacobian<1, dimension> Dcamera = boost::none,
                 OptionalJacobian<1, 6 + CalibrationB::dimension> Dother = boost::none) const
    {
        Matrix16 Dcamera_, Dother_;
        double   result = this->pose().range(camera.pose(), Dcamera ? &Dcamera_ : 0, Dother ? &Dother_ : 0);
        if (Dcamera)
        {
     ...
     * @return range (double)
     */
    double range(const CalibratedCamera& camera, OptionalJacobian<1, dimension> Dcamera = boost::none,
...
private:
    ...
    {
        ar& boost::serialization::make_nvp("PinholeBaseK", boost::serialization::base_object<Base>(*this));

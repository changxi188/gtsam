...
protected:
    ...
    static Vector ErrorVector(const ZVector& predicted, const ZVector& measured)
    {
        ...
        for (size_t i = 0, row = 0; i < m; i++, row += ZDim)
        {
            ...
            if (ZDim == 3 && std::isnan(bi(1)))
            {  // if it is a stereo point and the
...
public:
    ...
    static SymmetricBlockMatrix
    SchurComplement(const std::vector<Eigen::Matrix<double, ZDim, ND>,
                    ...
                    const Matrix& E, const Eigen::Matrix<double, N, N>& P, const Vector& b)
    {
        ...
        {  // for each camera
            ...
            augmentedHessian.setDiagonalBlock(i, FiT * (Fi - Ei_P * E.block(ZDim * i, 0, ZDim, N).transpose() * Fi));
            ...
            {  // for each camera
                ...
                // (DxD) = (Dx2) * ( (2x2) * (2xD) )
                augmentedHessian.setOffDiagonalBlock(i, j,
    ...
    static SymmetricBlockMatrix SchurComplementAndRearrangeBlocks(
        const std::vector<Eigen::Matrix<double, ZDim, ND>, Eigen::aligned_allocator<Eigen::Matrix<double, ZDim, ND>>>&
                      Fs,
        const Matrix& E, const Eigen::Matrix<double, N, N>& P, const Vector& b, const KeyVector& jacobianKeys,
        const KeyVector& hessianKeys)
    {
        ...
        else
        {
            ...
            augmentedHessianUniqueKeys =
                SymmetricBlockMatrix(dims, Matrix::Zero(NDD * nrUniqueKeys + 1, NDD * nrUniqueKeys + 1));
            ...
            {  // rows
                ...
                augmentedHessianUniqueKeys.updateOffDiagonalBlock(
                    keyToSlotMap[key_i], nrUniqueKeys, augmentedHessian.aboveDiagonalBlock(i, nrNonuniqueKeys));
                ...
                {  // cols
                    ...
                    if (i == j)
                    {
                        augmentedHessianUniqueKeys.updateDiagonalBlock(keyToSlotMap[key_i],
                                                                       augmentedHessian.diagonalBlock(i));
                    }
                    else
                    {  // (i < j)
                        ...
                        {
                            augmentedHessianUniqueKeys.updateOffDiagonalBlock(
                                keyToSlotMap[key_i], keyToSlotMap[key_j], augmentedHessian.aboveDiagonalBlock(i, j));
                        }
                        else
                        {
                            augmentedHessianUniqueKeys.updateDiagonalBlock(
                                keyToSlotMap[key_i], augmentedHessian.aboveDiagonalBlock(i, j) +
            ...
            augmentedHessianUniqueKeys.updateDiagonalBlock(nrUniqueKeys,
                                                           augmentedHessian.diagonalBlock(nrNonuniqueKeys));
        }
    ...
    static void ComputePointCovariance(Eigen::Matrix<double, N, N>& P, const Matrix& E, double lambda,
     ...
     * Dynamic version
     */
    static SymmetricBlockMatrix SchurComplement(const FBlocks& Fblocks, const Matrix& E, const Vector& b,
                                                const double lambda = 0.0, bool diagonalDamping = false)
    {
        if (E.cols() == 2)
        {
            Matrix2 P;
     ...
     */
    template <int N>  // N = 2 or 3 (point dimension)
    static void UpdateSchurComplement(const FBlocks& Fs, const Matrix& E, const Eigen::Matrix<double, N, N>& P,
                                      ...
                                      /*output ->*/ SymmetricBlockMatrix& augmentedHessian)
    {
        ...
        // Blockwise Schur complement
        for (size_t i = 0; i < m; i++)
        {  // for each camera in the current factor

            const MatrixZD&                   Fi   = Fs[i];
            const auto                        FiT  = Fi.transpose();
            const Eigen::Matrix<double, 2, N> Ei_P = E.template block<ZDim, N>(ZDim * i, 0) * P;
            ...
            augmentedHessian.updateDiagonalBlock(
                aug_i, ((FiT * (Fi - Ei_P * E.template block<ZDim, N>(ZDim * i, 0).transpose() * Fi))).eval());
            ...
            {  // for each camera
                ...
                // add contribution of current factor
                augmentedHessian.updateOffDiagonalBlock(
                    aug_i, aug_j, -FiT * (Ei_P * E.template block<ZDim, N>(ZDim * j, 0).transpose() * Fj));
            }
        }  // end of for over cameras

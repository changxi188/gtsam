...
/* ************************************************************************* */
Rot3::Rot3(double R11, double R12, double R13, double R21, double R22, double R23, double R31, double R32, double R33)
{
    Matrix3 R;
...
Rot3 Rot3::Rx(double t)
{
    ...
    return Rot3(1, 0, 0, 0, ct, -st, 0, st, ct);
}
...
{
    double st = sin(t), ct = cos(t);
    return Rot3(ct, 0, st, 0, 1, 0, -st, 0, ct);
}
...
{
    double st = sin(t), ct = cos(t);
    return Rot3(ct, -st, 0, st, ct, 0, 0, 0, 1);
}

/* ************************************************************************* */
// Considerably faster than composing matrices above !
Rot3 Rot3::RzRyRx(double x, double y, double z, OptionalJacobian<3, 1> Hx, OptionalJacobian<3, 1> Hy,
                  OptionalJacobian<3, 1> Hz)
{
    ...
    if (Hz)
        (*Hz) << -sy, sc_, cc_;
    return Rot3(_cc, -c_s + ssc, s_s + csc, _cs, c_c + sss, -s_c + css, -sy, sc_, cc_);
}
...
Rot3 Rot3::normalized() const
{
    ...
    if (std::fabs(rot.determinant() - 1) < 1e-12)
        return Rot3(rot_);
...
/* ************************************************************************* */
Point3 Rot3::rotate(const Point3& p, OptionalJacobian<3, 3> H1, OptionalJacobian<3, 3> H2) const
{
    if (H1)
        *H1 = rot_.matrix() * skewSymmetric(-p.x(), -p.y(), -p.z());
    if (H2)
        *H2 = rot_.matrix();
...
{
    if (H)
        throw std::runtime_error("Rot3::CayleyChart::Retract Derivative");
    const double x = omega(0), y = omega(1), z = omega(2);
    const double x2 = x * x, y2 = y * y, z2 = z * z;
    const double xy = x * y, xz = x * z, yz = y * z;
    const double f = 1.0 / (4.0 + x2 + y2 + z2), _2f = 2.0 * f;
    return Rot3((4 + x2 - y2 - z2) * f, (xy - 2 * z) * _2f, (xz + 2 * y) * _2f, (xy + 2 * z) * _2f,
                (4 - x2 + y2 - z2) * f, (yz - 2 * x) * _2f, (xz - 2 * y) * _2f, (yz + 2 * x) * _2f,
                (4 - x2 - y2 + z2) * f);
}
...
Rot3 Rot3::ChartAtOrigin::Retract(const Vector3& omega, ChartJacobian H)
{
    ...
    if (mode == Rot3::CAYLEY)
        return CayleyChart::Retract(omega, H);
    else
...
Vector3 Rot3::ChartAtOrigin::Local(const Rot3& R, ChartJacobian H)
{
    ...
    if (mode == Rot3::EXPMAP)
        return Logmap(R, H);
    if (mode == Rot3::CAYLEY)
        return CayleyChart::Local(R, H);
    else
        throw std::runtime_error("Rot3::Local: unknown mode");
}
...
/* ************************************************************************* */
Point3 Rot3::r1() const
{
    return Point3(rot_.matrix().col(0));
}

/* ************************************************************************* */
Point3 Rot3::r2() const
{
    return Point3(rot_.matrix().col(1));
}

/* ************************************************************************* */
Point3 Rot3::r3() const
{
    return Point3(rot_.matrix().col(2));
}

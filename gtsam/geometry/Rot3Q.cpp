...
Rot3::Rot3(const Point3& col1, const Point3& col2, const Point3& col3)
  : quaternion_((Matrix3() << col1.x(), col2.x(), col3.x(), col1.y(), col2.y(), col3.y(), col1.z(), col2.z(), col3.z())
                    .finished())
{
}
...
Rot3::Rot3(double R11, double R12, double R13, double R21, double R22, double R23, double R31, double R32, double R33)
  : quaternion_((Matrix3() << R11, R12, R13, R21, R22, R23, R31, R32, R33).finished())
{
}

/* ************************************************************************* */
Rot3::Rot3(const gtsam::Quaternion& q) : quaternion_(q)
{
}
                  ...
                  OptionalJacobian<3, 1> Hz)
{
    ...
    return Rot3(gtsam::Quaternion(Eigen::AngleAxisd(z, Eigen::Vector3d::UnitZ())) *
...
Point3 Rot3::rotate(const Point3& p, OptionalJacobian<3, 3> H1, OptionalJacobian<3, 3> H2) const
{
    ...
    if (H1)
        *H1 = R * skewSymmetric(-p.x(), -p.y(), -p.z());
    if (H2)
        *H2 = R;
    const Vector3 r = R * p;
...
{
    return Point3(quaternion_.toRotationMatrix().col(0));
}

/* ************************************************************************* */
Point3 Rot3::r2() const
{
    return Point3(quaternion_.toRotationMatrix().col(1));
}

/* ************************************************************************* */
Point3 Rot3::r3() const
{
    return Point3(quaternion_.toRotationMatrix().col(2));
}

...
public:
    enum
    {
        dimension = internal::DimensionSO(N)
    };
...
public:
    ...
    {
        if (n == 0)
            throw std::runtime_error("SO: Dimensionality not known.");
...
template <>
GTSAM_EXPORT Matrix SOn::Hat(const Vector& xi);

template <>
GTSAM_EXPORT Vector SOn::Vee(const Matrix& X);
...
template <>
GTSAM_EXPORT SOn LieGroup<SOn, Eigen::Dynamic>::compose(const SOn& g, DynamicJacobian H1, DynamicJacobian H2) const;

template <>
GTSAM_EXPORT SOn LieGroup<SOn, Eigen::Dynamic>::between(const SOn& g, DynamicJacobian H1, DynamicJacobian H2) const;

/*
 * Specialize dynamic vec.
 */
template <>
GTSAM_EXPORT typename SOn::VectorN2 SOn::vec(DynamicJacobian H) const;

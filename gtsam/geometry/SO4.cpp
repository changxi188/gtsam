...
//******************************************************************************
template <>
GTSAM_EXPORT Matrix4 SO4::Hat(const Vector6& xi)
{
...
//******************************************************************************
template <>
GTSAM_EXPORT Vector6 SO4::Vee(const Matrix4& X)
{
    Vector6 xi;
    xi(5) = -X(0, 1);
    xi(4) = +X(0, 2);
...
template <>
GTSAM_EXPORT SO4 SO4::Expmap(const Vector6& xi, ChartJacobian H)
{
    using namespace std;
    if (H)
        throw std::runtime_error("SO4::Expmap Jacobian");
    ...
    using std::abs;
    sort(e.data(), e.data() + 4, [](complex<double> a, complex<double> b) { return abs(a.imag()) > abs(b.imag()); });
    ...
    else if (a == b && b != 0)
    {
        ...
        double c0 = (a * sin_a + 2 * cos_a) / 2, c1 = (3 * sin_a - a * cos_a) / (2 * a), c2 = sin_a / (2 * a),
    ...
    else if (a != b)
    {
        ...
        double c0 = (b2 * cos_a - a2 * cos_b) / (b2 - a2), c1 = (b3 * sin_a - a3 * sin_b) / (a * b * (b2 - a2)),
               c2 = (cos_a - cos_b) / (b2 - a2), c3 = (b * sin_a - a * sin_b) / (a * b * (b2 - a2));
        return SO4(c0 * I_4x4 + c1 * X + c2 * X2 + c3 * X3);
    }
    else
    {
...
// so<4> generators
static std::vector<Matrix4, Eigen::aligned_allocator<Matrix4> >
    G4({SO4::Hat(Vector6::Unit(0)), SO4::Hat(Vector6::Unit(1)), SO4::Hat(Vector6::Unit(2)), SO4::Hat(Vector6::Unit(3)),
        SO4::Hat(Vector6::Unit(4)), SO4::Hat(Vector6::Unit(5))});

// vectorized generators
static const Eigen::Matrix<double, 16, 6> P4 =
    (Eigen::Matrix<double, 16, 6>() << vec4(G4[0]), vec4(G4[1]), vec4(G4[2]), vec4(G4[3]), vec4(G4[4]), vec4(G4[5]))
        .finished();

//******************************************************************************
template <>
GTSAM_EXPORT Matrix6 SO4::AdjointMap() const
{
...
//******************************************************************************
template <>
GTSAM_EXPORT SO4::VectorN2 SO4::vec(OptionalJacobian<16, 6> H) const
{
    const Matrix& Q = matrix_;
    if (H)
    {
        // As Luca calculated, this is (I4 \oplus Q) * P4
        *H << Q * P4.block<4, 6>(0, 0), Q * P4.block<4, 6>(4, 0), Q * P4.block<4, 6>(8, 0), Q * P4.block<4, 6>(12, 0);
    }
    return gtsam::vec4(Q);
}
...
///******************************************************************************
template <>
GTSAM_EXPORT SO4 SO4::ChartAtOrigin::Retract(const Vector6& xi, ChartJacobian H)
{
    if (H)
        throw std::runtime_error("SO4::ChartAtOrigin::Retract Jacobian");
    gttic(SO4_Retract);
...
//******************************************************************************
template <>
GTSAM_EXPORT Vector6 SO4::ChartAtOrigin::Local(const SO4& Q, ChartJacobian H)
{
    if (H)
        throw std::runtime_error("SO4::ChartAtOrigin::Retract Jacobian");
...
GTSAM_EXPORT Matrix3 topLeft(const SO4& Q, OptionalJacobian<9, 6> H)
{
    ...
    if (H)
    {
        const Vector3 m1 = M.col(0), m2 = M.col(1), m3 = M.col(2), q = R.topRightCorner<3, 1>();

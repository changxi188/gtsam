...
{
/* ************************************************************************* */
EssentialMatrix EssentialMatrix::FromRotationAndDirection(const Rot3& aRb, const Unit3& aTb, OptionalJacobian<5, 3> H1,
...
/* ************************************************************************* */
EssentialMatrix EssentialMatrix::FromPose3(const Pose3& aPb, OptionalJacobian<5, 6> H)
{
...
/* ************************************************************************* */
Point3 EssentialMatrix::transformTo(const Point3& p, OptionalJacobian<3, 5> DE, OptionalJacobian<3, 3> Dpoint) const
{
...
/* ************************************************************************* */
EssentialMatrix EssentialMatrix::rotate(const Rot3& cRb, OptionalJacobian<5, 5> HE, OptionalJacobian<5, 3> HR) const
{
    ...
    else
    {
        ...
        if (HR)
        {
            throw runtime_error("EssentialMatrix::rotate: derivative HR not implemented yet");
            /*
                              ...
                              OptionalJacobian<1, 5> H) const
{
    if (H)
    {
        ...
        Matrix12 HD = vA.transpose() * skewSymmetric(-rotation().matrix() * vB) * direction().basis();
        *H << HR, HD;
    }
    return dot(vA, E_ * vB);
}

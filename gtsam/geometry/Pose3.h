...
public:
    ...
    static Pose3 Create(const Rot3& R, const Point3& t, OptionalJacobian<6, 3> HR = boost::none,
     ...
     */
    Vector6 Adjoint(const Vector6& xi_b, OptionalJacobian<6, 6> H_this = boost::none,
                    OptionalJacobian<6, 6> H_xib = boost::none) const;

    /// The dual version of Adjoint
    Vector6 AdjointTranspose(const Vector6& x, OptionalJacobian<6, 6> H_this = boost::none,
     ...
     *
     * Let \f$ \hat{\xi}_i \f$ be the se3 Lie algebra, and \f$ \hat{\xi}_i^\vee = \xi_i = [\omega_i,v_i] \in
     * \mathbb{R}^6\f$ be its vector representation. We have the following relationship: \f$
     * [\hat{\xi}_1,\hat{\xi}_2]^\vee = ad_{\xi_1}(\xi_2) = [ad_{(\omega_1,v_1)}]*\xi_2 \f$
     *
     ...
     */
    Point3 transformFrom(const Point3& point, OptionalJacobian<3, 6> Hself = boost::none,
     ...
     */
    Point3 transformTo(const Point3& point, OptionalJacobian<3, 6> Hself = boost::none,
                       OptionalJacobian<3, 3> Hpoint = boost::none) const;

...
{
/* ************************************************************************* */
Cal3::Cal3(double fov, int w, int h) : s_(0), u0_((double)w / 2.0), v0_((double)h / 2.0)
{
...
/* ************************************************************************* */
void Cal3::print(const std::string& s) const
{
...
{
    return (std::fabs(fx_ - K.fx_) < tol && std::fabs(fy_ - K.fy_) < tol && std::fabs(s_ - K.s_) < tol &&
...
Matrix3 Cal3::inverse() const
{
    ...
    Matrix3      K_inverse;
    K_inverse << 1.0 / fx_, -s_ / fxy, (sv0 - fyu0) / fxy, 0.0, 1.0 / fy_, -v0_ / fy_, 0.0, 0.0, 1.0;
    return K_inverse;
}

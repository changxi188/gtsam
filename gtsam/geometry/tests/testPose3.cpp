...
TEST(Pose3, Adjoint_jacobians)
{
    ...
    std::function<Vector6(const Pose3&, const Vector6&)> Adjoint_proxy = [&](const Pose3& T, const Vector6& xi) {
        return T.Adjoint(xi);
    };
...
TEST(Pose3, AdjointTranspose)
{
    ...
    // Check evaluation
    EQUALITY(static_cast<Vector>(T.AdjointMap().transpose() * xi), T.AdjointTranspose(xi));
    EQUALITY(static_cast<Vector>(T2.AdjointMap().transpose() * xi), T2.AdjointTranspose(xi));
    EQUALITY(static_cast<Vector>(T3.AdjointMap().transpose() * xi), T3.AdjointTranspose(xi));

    // Check jacobians
    Matrix6                                              actualH1, actualH2, expectedH1, expectedH2;
    std::function<Vector6(const Pose3&, const Vector6&)> AdjointTranspose_proxy =
        [&](const Pose3& T, const Vector6& xi) { return T.AdjointTranspose(xi); };
...
TEST(Pose3, translation)
{
    ...
    Matrix numericalH =
        numericalDerivative11<Point3, Pose3>(std::bind(&Pose3::translation, std::placeholders::_1, boost::none), T);
...
TEST(Pose3, rotation)
{
    ...
    Matrix numericalH =
        numericalDerivative11<Rot3, Pose3>(std::bind(&Pose3::rotation, std::placeholders::_1, boost::none), T);
    EXPECT(assert_equal(numericalH, actualH, 1e-6));
}
...
TEST(Pose3, transform_to_with_derivatives)
{
    ...
    Matrix expH1 = numericalDerivative21(transform_to_, T, P), expH2 = numericalDerivative22(transform_to_, T, P);
...
TEST(Pose3, transform_from_with_derivatives)
{
    ...
    Matrix expH1 = numericalDerivative21(transformFrom_, T, P), expH2 = numericalDerivative22(transformFrom_, T, P);
    EXPECT(assert_equal(expH1, actH1, 1e-8));
    EXPECT(assert_equal(expH2, actH2, 1e-8));
}
...
{
    Point3 actual = Pose3(Rot3(), Point3(1, 2, 3)).transformTo(Point3(10., 20., 30.));
...
Pose3  xl1(Rot3::Ypr(0.0, 0.0, 0.0), Point3(1, 0, 0)), xl2(Rot3::Ypr(0.0, 1.0, 0.0), Point3(1, 1, 0)),
    xl3(Rot3::Ypr(1.0, 0.0, 0.0), Point3(2, 2, 0)), xl4(Rot3::Ypr(0.0, 0.0, 1.0), Point3(1, 4, -4));
...
TEST(Pose3, Align2)
{
    ...
    Point3           q1 = expected.transformFrom(p1), q2 = expected.transformFrom(p2), q3 = expected.transformFrom(p3);
...
TEST(Pose3, ExpmapDerivative1)
{
    ...
    Matrix expectedH = numericalDerivative21<Pose3, Vector6, OptionalJacobian<6, 6> >(&Pose3::Expmap, w, boost::none);
...
TEST(Pose3, LogmapDerivative)
{
    ...
    Matrix expectedH = numericalDerivative21<Vector6, Pose3, OptionalJacobian<6, 6> >(&Pose3::Logmap, p, boost::none);
    EXPECT(assert_equal(expectedH, actualH));
}
...
TEST(Pose3, adjoint)
{
    ...
    Matrix numericalH1 = numericalDerivative21<Vector6, Vector6, Vector6>(testDerivAdjoint, screwPose3::xi, v, 1e-5);
    Matrix numericalH2 = numericalDerivative22<Vector6, Vector6, Vector6>(testDerivAdjoint, screwPose3::xi, v, 1e-5);
...
TEST(Pose3, adjointTranspose)
{
    ...
    Matrix numericalH1 = numericalDerivative21<Vector6, Vector6, Vector6>(testDerivAdjointTranspose, xi, v, 1e-5);
    Matrix numericalH2 = numericalDerivative22<Vector6, Vector6, Vector6>(testDerivAdjointTranspose, xi, v, 1e-5);
...
TEST(Pose3, TransformCovariance6MapTo2d)
{
    ...
    auto match_cov3_to_cov2 = [&](int spatial_axis0, int spatial_axis1, int r_axis, const Pose2::Jacobian& cov2,
                                  const Pose3::Jacobian& cov3) -> void {
        EXPECT(
            assert_equal(Vector3{cov2.diagonal()}, Vector3{cov3(spatial_axis0, spatial_axis0),
                                                           cov3(spatial_axis1, spatial_axis1), cov3(r_axis, r_axis)}));
        EXPECT(assert_equal(
...
TEST(Pose3, TransformCovariance6)
{
    ...
    // rotate 90 around z axis and then 90 around y axis
    {
        ...
        EXPECT(assert_equal((Vector6{} << cov(1, 1), cov(2, 2), cov(0, 0), cov(4, 4), cov(5, 5), cov(3, 3)).finished(),
        ...
        EXPECT(assert_equal(
            ...
            (Vector5{} << transformed(1, 0), transformed(2, 0), transformed(3, 0), transformed(4, 0), transformed(5, 0))
                .finished()));
    }
    ...
    // rotate around x axis and translate along the x axis with uncertainty in roty
    {
        ...
        EXPECT(assert_equal((Vector6{} << 0., 0., 0.1 * 0.1, 0., 0.1 * 0.1 * 20. * 20., 0.).finished(),
                            Vector6{transformed.diagonal()}));
    }
}
...
{
    {
        ...
        double t = 0.5;
        Pose3 expectedPoseInterp(Rot3::Rz(M_PI_4), Point3(0.5, -0.207107, 0));  // note: different from test above: this
                                                                                // is full Pose3 interpolation
...
TEST(Pose3, Create)
{
    ...
    std::function<Pose3(Rot3, Point3)> create =
        std::bind(Pose3::Create, std::placeholders::_1, std::placeholders::_2, boost::none, boost::none);

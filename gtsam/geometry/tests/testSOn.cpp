...
TEST(SOn, RetractLocal)
{
    ...
    // Same for SO3:
    SOn I3(3);
    EXPECT(assert_equal(I3.retract(v1.tail<3>()), SOn(SO3::Expmap(v1.tail<3>()))));
    EXPECT(assert_equal(I3.retract(v2.tail<3>()), SOn(SO3::Expmap(v2.tail<3>()))));
...
{
    return SOn::VectorizedGenerators(n);
}
...
TEST(SOn, RetractJacobian)
{
    ...
    std::function<Matrix(const Vector&)> h = [](const Vector& v) { return SOn::ChartAtOrigin::Retract(v).matrix(); };
    Vector3                              v;
    v.setZero();

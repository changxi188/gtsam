...
TEST(SO3, ExpmapDerivative)
{
    ...
    const Matrix expectedDexpL = numericalDerivative11<Vector3, Vector3>(testDexpL, Vector3::Zero(), 1e-2);
...
TEST(SO3, ExpmapDerivative2)
{
    ...
    const Matrix  Jexpected =
        numericalDerivative11<SO3, Vector3>(std::bind(&SO3::Expmap, std::placeholders::_1, boost::none), theta);

    CHECK(assert_equal(Jexpected, SO3::ExpmapDerivative(theta)));
    CHECK(assert_equal(Matrix3(Jexpected.transpose()), SO3::ExpmapDerivative(-theta)));
}
...
TEST(SO3, ExpmapDerivative6)
{
    ...
    const Matrix  Jexpected =
        numericalDerivative11<SO3, Vector3>(std::bind(&SO3::Expmap, std::placeholders::_1, boost::none), thetahat);
    Matrix3 Jactual;
...
TEST(SO3, LogmapDerivative)
{
    ...
    const Matrix  Jexpected =
        numericalDerivative11<Vector, SO3>(std::bind(&SO3::Logmap, std::placeholders::_1, boost::none), R);
...
TEST(SO3, ApplyDexp)
{
    ...
    {
        std::function<Vector3(const Vector3&, const Vector3&)> f = [=](const Vector3& omega, const Vector3& v) {
            return so3::DexpFunctor(omega, nearZeroApprox).applyDexp(v);
        };
        for (Vector3 omega :
             {Vector3(0, 0, 0), Vector3(1, 0, 0), Vector3(0, 1, 0), Vector3(0, 0, 1), Vector3(0.1, 0.2, 0.3)})
        {
            so3::DexpFunctor local(omega, nearZeroApprox);
            for (Vector3 v : {Vector3(1, 0, 0), Vector3(0, 1, 0), Vector3(0, 0, 1), Vector3(0.4, 0.3, 0.2)})
            {
                EXPECT(assert_equal(Vector3(local.dexp() * v), local.applyDexp(v, aH1, aH2)));
...
TEST(SO3, ApplyInvDexp)
{
    ...
    for (bool nearZeroApprox : {true, false})
    {
             ...
             {Vector3(0, 0, 0), Vector3(1, 0, 0), Vector3(0, 1, 0), Vector3(0, 0, 1), Vector3(0.1, 0.2, 0.3)})
        {
            ...
            for (Vector3 v : {Vector3(1, 0, 0), Vector3(0, 1, 0), Vector3(0, 0, 1), Vector3(0.4, 0.3, 0.2)})
            {
                EXPECT(assert_equal(Vector3(invDexp * v), local.applyInvDexp(v, aH1, aH2)));
                EXPECT(assert_equal(numericalDerivative21(f, omega, v), aH1));
                EXPECT(assert_equal(numericalDerivative22(f, omega, v), aH2));
...
TEST(Matrix, compose)
{
    ...
    CHECK(assert_equal(expected, actual));
    std::function<Matrix3(const Matrix3&)> f          = [R](const Matrix3& M) { return so3::compose(M, R); };

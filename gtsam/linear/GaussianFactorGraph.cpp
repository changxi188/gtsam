...
SparseTriplets GaussianFactorGraph::sparseJacobian(const Ordering& ordering, size_t& nrows, size_t& ncols) const
{
    ...
    for (const auto& factor : *this)
    {
        ...
        if (!jacobianFactor)
        {
            ...
            if (hessian)
                ...
                throw std::invalid_argument("GaussianFactorGraph contains a factor that is neither a "
        ...
        for (size_t i = 0; i < (size_t)whitenedb.size(); i++)
        {
            ...
            if (std::abs(s) > 1e-12)
                entries.emplace_back(nrows + i, ncols, s);
        }
...
/* ************************************************************************* */
Matrix GaussianFactorGraph::augmentedJacobian(const Ordering& ordering) const
{
    // combine all factors
...
/* ************************************************************************* */
pair<Matrix, Vector> GaussianFactorGraph::jacobian(const Ordering& ordering) const
{
    Matrix augmented = augmentedJacobian(ordering);
    return make_pair(augmented.leftCols(augmented.cols() - 1), augmented.col(augmented.cols() - 1));
}
...
/* ************************************************************************* */
Matrix GaussianFactorGraph::augmentedHessian(const Ordering& ordering) const
{
...
/* ************************************************************************* */
pair<Matrix, Vector> GaussianFactorGraph::hessian(const Ordering& ordering) const
{
...
VectorValues GaussianFactorGraph::optimize(const Eliminate& function) const
{
    ...
    return BaseEliminateable::eliminateMultifrontal(Ordering::COLAMD, function)->optimize();
}
...
/* ************************************************************************* */
void GaussianFactorGraph::multiplyHessianAdd(double alpha, const VectorValues& x, VectorValues& y) const
{
...
void GaussianFactorGraph::printErrors(
    const VectorValues& values, const std::string& str, const KeyFormatter& keyFormatter,
    ...
    const
{
    ...
    for (size_t i = 0; i < (*this).size(); i++)
    {
        ...
        const double        errorValue = (factor != nullptr ? (*this)[i]->error(values) : .0);

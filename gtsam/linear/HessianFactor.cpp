...
/* ************************************************************************* */
HessianFactor::HessianFactor(Key j1, Key j2, const Matrix& G11, const Matrix& G12, const Vector& g1, const Matrix& G22,
                             const Vector& g2, double f)
  : GaussianFactor(KeyVector{j1, j2}), info_(Dims{G11.cols(), G22.cols(), 1})
{
...
/* ************************************************************************* */
HessianFactor::HessianFactor(Key j1, Key j2, Key j3, const Matrix& G11, const Matrix& G12, const Matrix& G13,
                             const Vector& g1, const Matrix& G22, const Matrix& G23, const Vector& g2,
                             const Matrix& G33, const Vector& g3, double f)
  : GaussianFactor(KeyVector{j1, j2, j3}), info_(Dims{G11.cols(), G22.cols(), G33.cols(), 1})
{
...
/* ************************************************************************* */
HessianFactor::HessianFactor(const KeyVector& js, const std::vector<Matrix>& Gs, const std::vector<Vector>& gs,
                             double f)
...
/* ************************************************************************* */
HessianFactor::HessianFactor(const GaussianFactorGraph& factors, const Scatter& scatter)
{
...
void HessianFactor::print(const std::string& s, const KeyFormatter& formatter) const
{
    ...
    cout << "\n";
    gtsam::print(Matrix(info_.selfadjointView()), "Augmented information matrix: ");
}
...
bool HessianFactor::equals(const GaussianFactor& lf, double tol) const
{
    ...
    if (!rhs || !Factor::equals(lf, tol))
        return false;
    return equal_with_abs_tol(augmentedInformation(), rhs->augmentedInformation(), tol);
}
...
/* ************************************************************************* */
Eigen::SelfAdjointView<SymmetricBlockMatrix::constBlock, Eigen::Upper> HessianFactor::informationView() const
{
...
{
    throw std::runtime_error("HessianFactor::hessianDiagonal raw memory access is allowed for Regular Factors only");
}
...
/* ************************************************************************* */
void HessianFactor::updateHessian(const KeyVector& infoKeys, SymmetricBlockMatrix* info) const
{
...
/* ************************************************************************* */
void HessianFactor::multiplyHessianAdd(double alpha, const VectorValues& x, VectorValues& yvalues) const
{
...
{
    throw std::runtime_error("HessianFactor::gradientAtZero raw memory access is allowed for Regular Factors only");
}

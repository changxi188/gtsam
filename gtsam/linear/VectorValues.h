...
public:
    ...
    VectorValues(std::initializer_list<std::pair<Key, Vector>> init) : values_(init.begin(), init.end())
    {
    }
    ...
    Vector& at(Key j)
    {
        ...
        if (item == end())
            throw std::out_of_range("Requested variable '" + DefaultKeyFormatter(j) + "' is not in this VectorValues.");
        else
            return item->second;
    }
    ...
    void erase(Key var)
    {
        if (values_.unsafe_erase(var) == 0)
            throw std::invalid_argument("Requested variable '" + DefaultKeyFormatter(var) +

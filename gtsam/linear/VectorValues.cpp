...
VectorValues::iterator VectorValues::insert(const std::pair<Key, Vector>& key_value)
{
    ...
    if (!result.second)
        throw std::invalid_argument("Requested to insert variable '" + DefaultKeyFormatter(key_value.first) +
...
VectorValues& VectorValues::update(const VectorValues& values)
{
    ...
    for (const KeyValuePair& key_value : values)
    {
        ...
        if (values_.size() > oldSize)
        {
            ...
            throw out_of_range("Requested to update a VectorValues with another VectorValues that "
...
VectorValues& VectorValues::insert(const VectorValues& values)
{
    ...
    if (size() != originalSize + values.size())
        throw invalid_argument("Requested to insert a VectorValues into another VectorValues that "
...
{
    return accumulate(combine(*this, other) | transformed(internal::structureCompareOp), true, logical_and<bool>());
}
...
VectorValues VectorValues::operator+(const VectorValues& c) const
{
    ...
    assert_throw(hasSameStructure(c), invalid_argument("VectorValues::operator+ called with different vector sizes"));
...
#endif
        ss << "    <tr>";
        ss << "<th>" << keyFormatter(kv.first) << "</th><td>" << kv.second.transpose() << "</td>";
        ss << "</tr>\n";
    }
    ss << "  </tbody>\n</table>\n</div>";
    return ss.str();

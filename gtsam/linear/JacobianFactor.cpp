...
/* ************************************************************************* */
JacobianFactor::JacobianFactor() : Ab_(Dims{1}, 0)
{
    getb().setZero();
}
...
/* ************************************************************************* */
JacobianFactor::JacobianFactor(Key i1, const Matrix& A1, const Vector& b, const SharedDiagonal& model)
{
    fillTerms(Pairs{{i1, A1}}, b, model);
}

/* ************************************************************************* */
JacobianFactor::JacobianFactor(const Key i1, const Matrix& A1, Key i2, const Matrix& A2, const Vector& b,
...
/* ************************************************************************* */
JacobianFactor::JacobianFactor(const Key i1, const Matrix& A1, Key i2, const Matrix& A2, Key i3, const Matrix& A3,
...
JacobianFactor::JacobianFactor(const HessianFactor& factor)
  : Base(factor), Ab_(VerticalBlockMatrix::LikeActiveViewOf(factor.info(), factor.rows()))
{
                    ...
                    else
                    {
                        if (!(varDims[jointVarpos] == vardim))
                        {
                            ...
                            ss << "Factor " << sourceFactorI << " variable "
                               ...
                               << " has different dimensionality of " << vardim << " instead of "
                               << varDims[jointVarpos];
...
/* ************************************************************************* */
FastVector<JacobianFactor::shared_ptr> _convertOrCastToJacobians(const GaussianFactorGraph& factors)
{
...
FastVector<VariableSlots::const_iterator> orderedSlotsHelper(const Ordering&      ordering,
                                                             const VariableSlots& variableSlots)
{
    ...
    if (nOrderingSlotsUsed != ordering.size())
        throw std::invalid_argument("The ordering provided to the JacobianFactor combine constructor\n"
...
/* ************************************************************************* */
JacobianFactor::JacobianFactor(const GaussianFactorGraph& graph, const VariableSlots& p_variableSlots)
{
    gttic(JacobianFactor_combine_constructor);
...
/* ************************************************************************* */
JacobianFactor::JacobianFactor(const GaussianFactorGraph& graph, const Ordering& ordering)
{
    gttic(JacobianFactor_combine_constructor);
    ...
    FastVector<VariableSlots::const_iterator> orderedSlots = orderedSlotsHelper(ordering, variableSlots);
                               ...
                               const VariableSlots& p_variableSlots)
{
    ...
    // Order variable slots
    FastVector<VariableSlots::const_iterator> orderedSlots = orderedSlotsHelper(ordering, p_variableSlots);
...
double JacobianFactor::error(const VectorValues& c) const
{
    ...
    if (model_)
        return 0.5 * model_->squaredMahalanobisDistance(e);
    return 0.5 * e.dot(e);
}
...
/* ************************************************************************* */
void JacobianFactor::updateHessian(const KeyVector& infoKeys, SymmetricBlockMatrix* info) const
{
    ...
    if (model && !model->isUnit())
    {
        if (model->isConstrained())
            throw invalid_argument("JacobianFactor::updateHessian: cannot update information with "
...
/* ************************************************************************* */
void JacobianFactor::transposeMultiplyAdd(double alpha, const Vector& e, VectorValues& x) const
{
    Vector E(e.size());
    E.noalias() = alpha * e;
    if (model_)
...
/* ************************************************************************* */
void JacobianFactor::multiplyHessianAdd(double alpha, const VectorValues& x, VectorValues& y) const
{
    Vector Ax = (*this) * x;
...
/* ************************************************************************* */
std::pair<GaussianConditional::shared_ptr, JacobianFactor::shared_ptr> EliminateQR(const GaussianFactorGraph& factors,
                                                                                   const Ordering&            keys)
{
    gttic(EliminateQR);
    ...
    {
        throw InvalidDenseElimination("EliminateQR was called with a request to eliminate variables that are not\n"
...
GaussianConditional::shared_ptr JacobianFactor::splitConditional(size_t nrFrontals)
{
    ...
    if (!model_)
    {
        throw std::invalid_argument("JacobianFactor::splitConditional cannot be  given a nullptr noise model");
    }
    ...
    conditionalNoiseModel = noiseModel::Diagonal::Sigmas(model_->sigmas().segment(Ab_.rowStart(), Ab_.rows()));
    GaussianConditional::shared_ptr conditional =
        boost::make_shared<GaussianConditional>(Base::keys_, nrFrontals, Ab_, conditionalNoiseModel);

    const DenseIndex maxRemainingRows = std::min(Ab_.cols(), originalRowEnd) - Ab_.rowStart() - frontalDim;

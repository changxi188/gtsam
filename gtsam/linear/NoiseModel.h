...
private:
    ...
    {
        // should never happen
        if (!sqrt_information_)
            throw std::runtime_error("Gaussian: has no R matrix");
...
public:
     ...
     */
    inline double sigma(size_t i) const
    {
        return sigmas_(i);
    }
     ...
     */
    inline const Vector& invsigmas() const
    {
        return invsigmas_;
    }
    inline double invsigma(size_t i) const
    {
        return invsigmas_(i);
    }
     ...
     * Return precisions
     */
    inline const Vector& precisions() const
    {
        return precisions_;
    }
    inline double precision(size_t i) const
    {
...
protected:
    ...
    Isotropic(size_t dim, double sigma) : Diagonal(Vector::Constant(dim, sigma)), sigma_(sigma), invsigma_(1.0 / sigma)
    {
    }
...
/// traits
template <>
struct traits<noiseModel::Gaussian> : public Testable<noiseModel::Gaussian>
{
};
template <>
struct traits<noiseModel::Diagonal> : public Testable<noiseModel::Diagonal>
{
};
template <>
struct traits<noiseModel::Constrained> : public Testable<noiseModel::Constrained>
{
};
template <>
struct traits<noiseModel::Isotropic> : public Testable<noiseModel::Isotropic>
{
};
template <>
struct traits<noiseModel::Unit> : public Testable<noiseModel::Unit>
{
};

...
{
protected:
    SymmetricBlockMatrix info_;  ///< The full augmented information matrix, s.t. the quadratic error is 0.5*[x
                                 ///< -1]'*H*[x -1]
...
public:
     ...
     * So, with \f$ A = [A1 A2] \f$ and \f$ G=A*'M*A = [A1';A2']*M*[A1 A2] \f$ we have
     ...
     \endcode
     */
    HessianFactor(Key j1, Key j2, const Matrix& G11, const Matrix& G12, const Vector& g1, const Matrix& G22,
     ...
     * term, and f the constant term.
     */
    HessianFactor(Key j1, Key j2, Key j3, const Matrix& G11, const Matrix& G12, const Matrix& G13, const Vector& g1,
                  const Matrix& G22, const Matrix& G23, const Vector& g2, const Matrix& G33, const Vector& g3,
                  double f);
    ...
    explicit HessianFactor(const GaussianFactorGraph& factors, const Scatter& scatter);

    /** Combine a set of factors into a single dense HessianFactor */
    explicit HessianFactor(const GaussianFactorGraph& factors) : HessianFactor(factors, Scatter(factors))
    {
    }
    ...
    double& constantTerm()
    {
        return info_.diagonalBlock(size())(0, 0);
    }

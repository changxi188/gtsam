...
Matrix Gaussian::information() const
{
...
// Diagonal
/* ************************************************************************* */
Diagonal::Diagonal() : Gaussian(1)  // TODO: Frank asks: really sure about this?
{
}
...
/* ************************************************************************* */
Diagonal::shared_ptr Diagonal::Precisions(const Vector& precisions, bool smart)
{
...
/* ************************************************************************* */
Constrained::Constrained(const Vector& sigmas) : Diagonal(sigmas), mu_(Vector::Constant(sigmas.size(), 1000.0))
{
    internal::fix(sigmas, precisions_, invsigmas_);
}

/* ************************************************************************* */
Constrained::Constrained(const Vector& mu, const Vector& sigmas) : Diagonal(sigmas), mu_(mu)
{
    internal::fix(sigmas, precisions_, invsigmas_);
}

/* ************************************************************************* */
Constrained::shared_ptr Constrained::MixedSigmas(const Vector& mu, const Vector& sigmas)
{
...
Constrained::shared_ptr Constrained::MixedSigmas(double m, const Vector& sigmas)
{
    return MixedSigmas(Vector::Constant(sigmas.size(), m), sigmas);
}

Constrained::shared_ptr Constrained::MixedVariances(const Vector& mu, const Vector& variances)
{
...
Constrained::shared_ptr Constrained::MixedPrecisions(const Vector& mu, const Vector& precisions)
{
    ...
    for (size_t j = 0; j < n; ++j)
    {
        ...
        else
        {
            ...
            for (size_t i = 0; i < m; i++)
            {
                double ai = a(i, 0);
                if (std::abs(ai) > 1e-9)
                {  // also catches remaining sigma==0 rows
...
{
    if (smart && std::abs(sigma - 1.0) < 1e-9)
        return Unit::Create(dim);
...
{
    if (smart && std::abs(variance - 1.0) < 1e-9)
        return Unit::Create(dim);
...
Robust::shared_ptr Robust::Create(const RobustModel::shared_ptr& robust, const NoiseModel::shared_ptr noise)
{

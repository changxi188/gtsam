...
bool BatchFixedLagSmoother::equals(const FixedLagSmoother& rhs, double tol) const
{
    ...
    return e != nullptr && FixedLagSmoother::equals(*e, tol) && factors_.equals(e->factors_, tol) &&
...
{
    throw runtime_error("BatchFixedLagSmoother::marginalCovariance not implemented");
}

/* ************************************************************************* */
FixedLagSmoother::Result BatchFixedLagSmoother::update(const NonlinearFactorGraph& newFactors, const Values& newTheta,
...
void BatchFixedLagSmoother::removeFactors(const set<size_t>& deleteFactors)
{
    for (size_t slot : deleteFactors)
    {
        ...
        {
            // TODO: Throw an error??
            cout << "Attempting to remove a factor from slot " << slot << ", but it is already nullptr." << endl;
        }
    }
}
...
FixedLagSmoother::Result BatchFixedLagSmoother::optimize()
{
    ...
    do
    {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    if (lambda >= lambdaUpperBound)
                    {
                        ...
                        {
                            cout << "Warning:  Levenberg-Marquardt giving up because cannot decrease error with "
                                    "maximum lambda"
                                 << endl;
                        }
                        break;
        ...
        result.iterations++;
             ...
             !checkConvergence(relativeErrorTol, absoluteErrorTol, errorTol, previousError, result.error,
...
void BatchFixedLagSmoother::marginalize(const KeyVector& marginalizeKeys)
{
    ...
    NonlinearFactorGraph marginalFactors =
        CalculateMarginalFactors(removedFactors, theta_, marginalizeKeys, parameters_.getEliminationFunction());
...
/* ************************************************************************* */
void BatchFixedLagSmoother::PrintKeySet(const set<Key>& keys, const string& label)
{
    cout << label;
    for (Key key : keys)
    {
...
/* ************************************************************************* */
void BatchFixedLagSmoother::PrintKeySet(const KeySet& keys, const string& label)
{
    cout << label;
    for (Key key : keys)
    {
...
/* ************************************************************************* */
void BatchFixedLagSmoother::PrintSymbolicGraph(const NonlinearFactorGraph& graph, const string& label)
{
    cout << label << endl;
...
/* ************************************************************************* */
void BatchFixedLagSmoother::PrintSymbolicGraph(const GaussianFactorGraph& graph, const string& label)
{
    cout << label << endl;
    for (const auto& factor : graph)
    {
...
GaussianFactorGraph BatchFixedLagSmoother::CalculateMarginalFactors(
    const GaussianFactorGraph& graph, const KeyVector& keys, const GaussianFactorGraph::Eliminate& eliminateFunction)
{
    if (keys.size() == 0)
    {
    ...
    const GaussianFactorGraph::Eliminate& eliminateFunction)
{
    ...
    else
    {
        ...
        const auto marginalLinearFactors = CalculateMarginalFactors(*linearFactorGraph, keys, eliminateFunction);

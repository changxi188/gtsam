...
protected:
     ...
     */
    PartialPriorFactor(Key key, const SharedNoiseModel& model) : Base(model, key)
    {
    }
...
public:
    ...
    PartialPriorFactor(Key key, size_t idx, double prior, const SharedNoiseModel& model)
      : Base(model, key), prior_((Vector(1) << prior).finished()), indices_(1, idx)
    {
        assert(model->dim() == 1);
    }
    ...
    bool equals(const NonlinearFactor& expected, double tol = 1e-9) const override
    {
        ...
        return e != nullptr && Base::equals(*e, tol) && gtsam::equal_with_abs_tol(this->prior_, e->prior_, tol) &&

...
ISAM2 createSlamlikeISAM2(boost::optional<Values&>               init_values = boost::none,
                          ...
                          const ISAM2Params& params   = ISAM2Params(ISAM2GaussNewtonParams(0.001), 0.0, 0, false, true,
                                                                  ISAM2Params::CHOLESKY, true, DefaultKeyFormatter,
                                                                  true),
                          size_t             maxPoses = 10)
{
    ...
    // Add odometry from time 10 to 11 and landmark measurement at time 10
    {
        ...
        newfactors += BetweenFactor<Pose2>(i, i + 1, Pose2(1.0, 0.0, 0.0), odoNoise);
        newfactors +=
            BearingRangeFactor<Pose2, Point2>(i, 100, Rot2::fromAngle(M_PI / 4.0 + M_PI / 16.0), 4.5, brNoise);
        newfactors +=
            BearingRangeFactor<Pose2, Point2>(i, 101, Rot2::fromAngle(-M_PI / 4.0 + M_PI / 16.0), 4.5, brNoise);
        fullgraph.push_back(newfactors);
...
struct ConsistencyVisitor
{
    bool         consistent;
    const ISAM2& isam;
    ConsistencyVisitor(const ISAM2& isam) : consistent(true), isam(isam)
    {
    }
    int operator()(const ISAM2::sharedClique& node, int& parentData)
    {
        if (find(isam.roots().begin(), isam.roots().end(), node) == isam.roots().end())
        {
            if (node->parent_.expired())
                consistent = false;
            if (find(node->parent()->children.begin(), node->parent()->children.end(), node) ==
...
/* ************************************************************************* */
bool isam_check(const NonlinearFactorGraph& fullgraph, const Values& fullinit, const ISAM2& isam, Test& test,
                TestResult& result)
{
    ...
    for (const sharedClique& clique : isam.nodes() | br::map_values)
    {
        ...
        for (GaussianConditional::const_iterator jit = clique->conditional()->begin();
             jit != clique->conditional()->end(); ++jit)
        {
...
TEST(ISAM2, simple)
{
    for (size_t i = 0; i < 10; ++i)
    {
        ...
        NonlinearFactorGraph fullgraph;
        ISAM2                isam =
            createSlamlikeISAM2(fullinit, fullgraph, ISAM2Params(ISAM2GaussNewtonParams(0.001), 0.0, 0, false), i);
...
TEST(ISAM2, slamlike_solution_gaussnewton_qr)
{
    ...
    ISAM2                isam = createSlamlikeISAM2(fullinit, fullgraph,
                                     ISAM2Params(ISAM2GaussNewtonParams(0.001), 0.0, 0, false, false, ISAM2Params::QR));
...
TEST(ISAM2, slamlike_solution_dogleg_qr)
{
    ...
    NonlinearFactorGraph fullgraph;
    ISAM2                isam = createSlamlikeISAM2(fullinit, fullgraph,
                                     ISAM2Params(ISAM2DoglegParams(1.0), 0.0, 0, false, false, ISAM2Params::QR));
...
TEST(ISAM2, clone)
{
...

    {
        ...
        factors +=
            BetweenFactor<Pose2>(0, 10, isam.calculateEstimate<Pose2>(0).between(isam.calculateEstimate<Pose2>(10)),
...
TEST(ISAM2, swapFactors)
{
    ...
    // Remove the measurement on landmark 0 and replace with a different one
    {
        ...
        //    swapfactors += BearingRange<Pose2,Point2>(10, 100, Rot2::fromAngle(M_PI/4.0 + M_PI/16.0), 4.5, brNoise; //
        //    original factor
        swapfactors +=
            BearingRangeFactor<Pose2, Point2>(10, 100, Rot2::fromAngle(M_PI / 4.0 + M_PI / 16.0), 5.0, brNoise);
...
bool checkMarginalizeLeaves(ISAM2& isam, const FastList<Key>& leafKeys)
{
    ...
    expected3AugmentedHessian = isam.getFactorsUnsafe()
                                    .linearize(isam.getLinearizationPoint())
                                    ->marginal(toKeep, EliminateQR)
                                    ->augmentedHessian();
    ...
    Matrix actual3AugmentedHessian =
        isam.getFactorsUnsafe().linearize(isam.getLinearizationPoint())->augmentedHessian();
    ...
    // actualAugmentedHessian.bottomRightCorner(1,1) = expected2AugmentedHessian.bottomRightCorner(1,1); bool linEqual =
    // assert_equal(expected2AugmentedHessian, actualAugmentedHessian, 1e-6); cout << "nonlinEqual" << endl;
    ...
    // actual2AugmentedHessian.bottomRightCorner(1,1) = expected3AugmentedHessian.bottomRightCorner(1,1); bool
    // afterLinCorrect = assert_equal(expected3AugmentedHessian, actual2AugmentedHessian, 1e-6); cout << "nonlinCorrect"
    // << endl;

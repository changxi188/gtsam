...
po::variables_map parseOptions(int argc, char* argv[])
{
    ...
    desc.add_options()("help,h", "produce help message")("data_csv_path",
                                                         po::value<string>()->default_value("imuAndGPSdata.csv"),
                                                         "path to the CSV file with the IMU data")(
        "output_filename", po::value<string>()->default_value("imuFactorExampleResults.csv"),
        "path to the result file to use")("use_isam", po::bool_switch(), "use ISAM as the optimizer");
...
int main(int argc, char* argv[])
{
    ...
    while (file.good())
    {
        ...
        {  // IMU measurement
            ...
            const Vector3 first_3 = imu.head<3>();
            const Vector3 tail_3  = imu.tail<3>();
            // preintegrated->integrateMeasurement(imu.head<3>(), imu.tail<3>(), dt);
            preintegrated->integrateMeasurement(first_3, tail_3, dt);
        }
        else if (type == 1)
        {  // GPS measurement
            Vector7 gps;
            ...
            fprintf(fp_out, "%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n", output_time, gtsam_position(0),
                    gtsam_position(1), gtsam_position(2), gtsam_quat.x(), gtsam_quat.y(), gtsam_quat.z(),
                    gtsam_quat.w(), gps(0), gps(1), gps(2), gps_quat.x(), gps_quat.y(), gps_quat.z(), gps_quat.w());

...
void PrintDuration(const std::chrono::nanoseconds dur, double num_samples, const std::string& method)
{
    ...
    std::cout << "Time taken by " << method << ": " << nanoseconds * 1e-3 << std::endl;
}

void GetLargeCamerasDataset(CameraSet<PinholeCamera<Cal3_S2>>* cameras, std::vector<Pose3>* poses, Point3* point,
...
void GetSmallCamerasDataset(CameraSet<PinholeCamera<Cal3_S2>>* cameras, std::vector<Pose3>* poses, Point3* point,
                            Point2Vector* measurements)
{
    Pose3                  pose1;
...
int main(int argc, char* argv[])
{
    ...
    SharedNoiseModel measurementNoise = noiseModel::Isotropic::Sigma(2, measurementSigma);
    ...
    {
        Point2Vector noisyMeasurements = AddNoiseToMeasurements(measurements, measurementSigma);

        auto                    lostStart = std::chrono::high_resolution_clock::now();
        boost::optional<Point3> estimateLOST =
            triangulatePoint3<Cal3_S2>(cameras, noisyMeasurements, rank_tol, false, measurementNoise, true);
        ...
        boost::optional<Point3> estimateDLT =
            triangulatePoint3<Cal3_S2>(cameras, noisyMeasurements, rank_tol, false, measurementNoise, false);
        durationDLT += std::chrono::high_resolution_clock::now() - dltStart;

        auto                    dltOptStart = std::chrono::high_resolution_clock::now();
        boost::optional<Point3> estimateDLTOpt =
            triangulatePoint3<Cal3_S2>(cameras, noisyMeasurements, rank_tol, true, measurementNoise, false);
